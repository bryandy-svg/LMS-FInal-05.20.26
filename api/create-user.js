...
function friendlyCreateUserError(error) {
  const raw = `${error?.message || ""} ${JSON.stringify(error?.body || {})}`;
  if (/not_admin|user not allowed|403/i.test(raw)) {
    return "Supabase rejected user creation because Vercel is not using the service-role key. In Vercel, set SUPABASE_SERVICE_ROLE_KEY to the Supabase legacy service_role secret key, not the anon or publishable key, then redeploy.";
  }
  return error.message || "Could not save user.";
}
...
async function assertCanManageUsers(req) {
  ...
  if (isBryanOwner(user)) return;
  const profile = await requesterProfile(user);
  if (isBryanOwner({ ...profile, email: user?.email })) return;
...
function isBryanOwner(userOrProfile = {}) {
  const email = String(userOrProfile.email || "").trim().toLowerCase();
  const username = String(userOrProfile.username || userOrProfile.user_metadata?.username || userOrProfile.user_metadata?.full_name || "").trim().toLowerCase();
  return email === "bryan.dy@lmsfm.com" || username === "bryan.dy";
}

async function requesterProfile(user) {
  const email = String(user?.email || "").trim();
  const username = String(user?.user_metadata?.username || user?.user_metadata?.full_name || "").trim();
  const byId = await supa(`/rest/v1/app_profiles?id=eq.${encodeURIComponent(user.id)}&select=role,modules`);
  if (byId?.[0]) return byId[0];
  if (email) {
    const byEmail = await supa(`/rest/v1/app_profiles?email=ilike.${encodeURIComponent(email)}&select=role,modules`);
    if (byEmail?.[0]) return byEmail[0];
    const byUsernameEmail = await supa(`/rest/v1/app_profiles?username=ilike.${encodeURIComponent(email)}&select=role,modules`);
    if (byUsernameEmail?.[0]) return byUsernameEmail[0];
  }
  if (username) {
    const byUsername = await supa(`/rest/v1/app_profiles?username=ilike.${encodeURIComponent(username)}&select=role,modules`);

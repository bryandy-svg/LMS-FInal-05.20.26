...
const tableMap = {
  ...
  equipmentrequests: { table: "equipment_requests", key: "request_no", title: "Equipment Requests", sub: "Request equipment with date range, location, customer PO, email, signature, and approval tracking.", heads: ["request_no", "request_date", "asset_tag", "asset_name", "location", "requested_by", "email_address", "po_no", "from_date", "to_date", "approved_by", "status"], labels: ["Request #", "Date", "Asset #", "Equipment", "Location", "Requested By", "Email", "PO #", "From", "To", "Approved By", "Status"] },
...
async function openEquipmentRequestModal(row = null) {
  ...
  $("modalBody").innerHTML = `
    <div class="form-grid equipment-request-form">
      ...
      ${productSelect("Location", "location", productMeta.locations || [], row?.location || "", "New location")}
...
async function saveEquipmentRequestModal() {
  ...
  try {
    await upsertOneWithOptionalColumns("equipment_requests", record, "request_no", ["location", "email_address"], "Saved the equipment request. Location/email columns need the latest equipment request database update before they can be stored.");
...
async function approveEquipmentRequest(requestNo) {
  ...
  try {
    ...
    await updateOneWithOptionalColumns("assets", {
      ...
      approved_by: approver,
      location: request.location || asset.location || null,
    ...
    notifyEquipmentRequestApproved(request, woNo, approver);
    await renderEquipmentRequestsView();
  } catch (error) {
...
function notifyEquipmentRequestApproved(request, woNo, approver) {
  ...
  const body = [
    ...
    request.asset_tag ? `Asset: ${request.asset_tag} ${request.asset_name || ""}` : "",
    request.location ? `Location: ${request.location}` : "",
    request.from_date || request.to_date ? `Requested dates: ${request.from_date || ""} to ${request.to_date || ""}` : "",
    woNo ? `Pre Delivery Inspection work order: ${woNo}` : "",
...
async function importLocalState(local) {
  ...
  await upsertMany("equipment_requests", (local.equipmentRequests || []).map((r, i) => {
    ...
    return {
      ...
      location: r.location || asset?.location || null,

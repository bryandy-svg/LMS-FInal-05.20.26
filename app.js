...
const tableMap = {
  ...
  equipmentrequests: { table: "equipment_requests", key: "request_no", title: "Equipment Requests", sub: "Request equipment with date range, customer PO, signature, and approval tracking.", heads: ["request_no", "request_date", "asset_tag", "asset_name", "requested_by", "po_no", "from_date", "to_date", "approved_by", "status"], labels: ["Request #", "Date", "Asset #", "Equipment", "Requested By", "PO #", "From", "To", "Approved By", "Status"] },
...
const assetColumnDefs = [
  ...
  ["last_update_date", "Last Update Date"], ["scanned_date", "Scanned Date"], ["reading", "Reading"], ["assigned_operator", "Operator"], ["requested_by", "Requested By"], ["approved_by", "Approved By"], ["repair_po_no", "Repair PO #"], ["open_wo_issue", "WO / Issue"], ["status", "Status"],
];
...
function isSuggestPicker(el) {
  return !!el?.matches?.("input.suggest-input[list]");
}

function prepareSuggestDropdown(input) {
  ...
  if (document.activeElement !== input || input.dataset.pickerCleared !== "1") {
    input.dataset.previousValue = current;
    input.dataset.pickerCleared = current ? "1" : "";
    input.dataset.pickerDirty = "";
    if (current) {
      input.classList.add("picker-open");
      input.value = "";
      input.setAttribute("placeholder", `Current: ${current}`);
    }
  }
  setTimeout(() => {
    try {
      ...
      // Browser support varies; the cleared field still makes datalist choices easier to reach.
    }
  }, 0);
}

function restoreSuggestDropdown(input, force = false) {
  if (!isSuggestPicker(input) || input.dataset.pickerCleared !== "1") return;
  const shouldRestore = force || (!input.dataset.pickerDirty && !input.value);
  if (shouldRestore) input.value = input.dataset.previousValue || "";
  input.classList.remove("picker-open");
  input.setAttribute("placeholder", input.dataset.originalPlaceholder || "");
  input.dataset.pickerCleared = "";
  input.dataset.pickerDirty = "";
  input.dataset.previousValue = "";
}

function restorePendingSuggestInputs(scope = document) {
  scope.querySelectorAll?.("input.suggest-input[list]").forEach((input) => restoreSuggestDropdown(input));
}
...
function setupSeamlessDropdowns() {
  ...
  document.addEventListener("pointerdown", (event) => {
    if (!isSuggestPicker(event.target)) restorePendingSuggestInputs();
    prepareSuggestDropdown(event.target);
  }, true);
  document.addEventListener("focusin", (event) => prepareSuggestDropdown(event.target));
  document.addEventListener("input", (event) => {
    if (isSuggestPicker(event.target)) event.target.dataset.pickerDirty = "1";
  });
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && isSuggestPicker(event.target)) {
      restoreSuggestDropdown(event.target, true);
      event.target.blur();
    }
  });
  document.addEventListener("focusout", (event) => {
    if (isSuggestPicker(event.target)) setTimeout(() => restoreSuggestDropdown(event.target), 0);
  });
  document.addEventListener("click", (event) => {
    if (event.target?.id === "modalSave" || event.target?.closest?.("[data-save], .primary")) {
      restorePendingSuggestInputs();
    }
  }, true);
}
...
const trainingTours = {
  ...
  full: [
    ...
    { view: "repairs", title: "Repairs and work orders", body: "Repairs are separated into Open, Ready to Close, Closed Not Invoiced, and Invoiced work orders. Invoiced work orders are locked unless reversed.", target: "#repairTableHost" },
...
function updateAuthView() {
  ...
  ["importBtn", "sampleBtn", "clearBtn", "exportBtn"].forEach((id) => {
    if ($(id)) $(id).style.display = session && (isMechanicUser() || isEquipmentRequestUser()) ? "none" : "";
  });
}
...
async function loadView(view) {
  ...
  if (view === "outsidefleet") return renderOutsideCustomerFleetView();
  if (view === "equipmentrequests") return renderEquipmentRequestsView();
...
function accountsPayableRows(data) {
  return data.pos.map((po) => {
    ...
    const apPosted = isBeginningAp || isLandedAp || data.gl.some((row) => row.reference === po.po_no && ["Purchase Order", "Landed Cost Invoice"].includes(row.source));
...
function poApSummary(po) {
  ...
  const isBeginningAp = /beginning ap/i.test(`${po.status || ""} ${po.notes || ""}`);
  const isLandedAp = isLandedCostPayable(po);
  const match = isBeginningAp || isLandedAp ? "Matched" : po.match_status && !/pending|awaiting/i.test(po.match_status) ? po.match_status : !received ? "Awaiting Goods" : !invoiceAmount ? "Pending" : Math.abs(invoiceAmount - received) <= 0.005 ? "Matched" : "Mismatch";
...
async function renderProductsView() {
  ...
  $("content").innerHTML = `
    ...
    <section class="panel">
      <div class="panel-head"><div class="panel-title"><strong>Product Master</strong><span>${currentRows.length} items shown. Master data only; stock changes post from documents.</span></div><div class="actions"><button id="productColumnsBtn">Columns</button><button id="productTemplateBtn">Download product template</button><button id="productUploadBtn">Upload product list</button><button id="productsCsvBtn">Excel</button><button onclick="window.print()">PDF / Print</button></div></div>
...
function linkedLandedSourcePoNo(po) {
  const match = String(po?.notes || "").match(/Linked landed cost PO:\s*([^\n]+)/i);
...
async function openPurchaseOrderModal(po = null) {
  ...
  $("modalBody").innerHTML = `
    ...
    <p class="notice">Jobsite / Project, vendor, terms, and lines are required. Landed cost amounts are internal cost estimates for inventory valuation. If the freight forwarder, customs broker, or duty bill is a separate vendor invoice, use Landed AP after the PO is saved.</p>`;
...
async function openLandedCostInvoiceModal(sourcePo) {
  if (!sourcePo) return;
  if (!sourcePo.landed_cost_enabled && !poLandedAddOn(sourcePo)) {
    alert("Turn on landed cost or enter estimated freight/duty/other landed cost on the product PO first.");
    return;
  }
  const refNo = await nextLandedCostRef();
  const estimatedRemaining = Math.max(0, poLandedAddOn(sourcePo) - landedCostApTotalForSource(sourcePo.po_no));
  $("modalTitle").textContent = `Landed cost AP for ${sourcePo.po_no}`;
  $("modalBody").innerHTML = `
    <div class="form-grid">
      ${productInput("Landed AP #", "po_no", refNo)}
      ${productInput("Source product PO", "source_po_no", sourcePo.po_no)}
      ...
      ${productInput("Invoice amount", "vendor_invoice_amount", estimatedRemaining || poLandedAddOn(sourcePo), "number")}
      ${productSelect("Payment status", "payment_status", ["Ready to Pay", "Hold", "Paid"], "Ready to Pay")}
      <div class="field wide"><label>Notes</label><textarea data-product-field="notes">Separate landed cost invoice for ${esc(sourcePo.po_no)}. This clears Landed Cost Accrual and creates AP to the selected vendor.</textarea></div>
    </div>
    <p class="notice">Use this when the product supplier invoice and freight/customs invoice are different vendors. The product PO keeps its own AP. This landed AP creates a separate payable while the received inventory keeps the landed FIFO cost.</p>`;
  $("modalSave").onclick = async () => {
    const record = collectProductModalFields();
    ...
    try {
      const landedPo = {
        ...
        jobsite_project: sourcePo.jobsite_project || `Landed cost for ${sourcePo.po_no}`,
        ...
        notes: [`Linked landed cost PO: ${sourcePo.po_no}`, `Cost type: ${record.cost_type || "Other"}`, record.notes || ""].filter(Boolean).join("\n"),
      };
      ...
      await postLandedCostInvoiceLedger(saved, sourcePo.po_no, record.cost_type || "Other");
      closeModal();
      renderPurchasingView();
    } catch (error) {
      alert(error.message || error);
...
function landedCostApTotalForSource(sourcePoNo) {
  return currentRows
    .filter((row) => isLandedCostPayable(row) && linkedLandedSourcePoNo(row) === sourcePoNo && !/cancel/i.test(row.status || ""))
    .reduce((sum, row) => sum + Number(row.vendor_invoice_amount || 0), 0);
}
...
async function postLandedCostInvoiceLedger(landedPo, sourcePoNo, costType = "Other") {
  ...
  await upsertMany("general_ledger", [
    { entry_date: postingDate, posting_date: postingDate, account: "Landed Cost Accrual", vendor: landedPo.vendor, invoice_no: landedPo.vendor_invoice_no, invoice_date: landedPo.vendor_invoice_date, due_date: landedPo.due_date, description: `Clear landed cost accrual ${sourcePoNo} - ${costType}`, reference: landedPo.po_no, debit: amount, credit: 0, source: "Landed Cost Invoice", status: "Posted" },
    { entry_date: postingDate, posting_date: postingDate, account: "Accounts Payable (A/P)", vendor: landedPo.vendor, invoice_no: landedPo.vendor_invoice_no, invoice_date: landedPo.vendor_invoice_date, due_date: landedPo.due_date, description: `Landed cost payable for ${sourcePoNo}`, reference: landedPo.po_no, debit: 0, credit: amount, source: "Landed Cost Invoice", status: "Posted" },
  ], "id");
}
...
async function saveNewWorkOrderModal() {
  ...
  const existingOpen = workOrders.find((wo) => wo.wo_no && String(wo.asset_tag || "").toLowerCase() === String(record.asset_tag || "").toLowerCase() && !wo.invoice_no && !/closed|complete|invoiced|cancel/i.test(wo.status || ""));
  if (existingOpen) {
...
async function renderOutsideCustomerFleetView() {
  ...
  currentRows = rows.map((row) => {
    const openWo = workOrders.find((wo) => [row.reference, row.vin].filter(Boolean).some((key) => String(wo.asset_tag || "").toLowerCase() === String(key || "").toLowerCase()) && !/closed|complete|invoiced|cancel/i.test(wo.status || ""));
...
async function renderEquipmentRequestsView() {
  ...
  $("newEquipmentRequestBtn").onclick = () => openEquipmentRequestModal();
    ...
    <section class="request-portal">
      <div class="request-hero">
        <div>
          ...
          <p>Search equipment, request the dates needed, sign, and submit.</p>
        </div>
        <button class="primary" id="mobileNewEquipmentRequestBtn">New request</button>
      </div>
      <input class="searchbox" id="equipmentRequestSearch" placeholder="Search request, asset, PO, status">
      <div id="equipmentRequestHost" class="request-card-list">${equipmentRequestCards(currentRows)}</div>
    </section>`;
  $("mobileNewEquipmentRequestBtn").onclick = () => openEquipmentRequestModal();
  $("equipmentRequestSearch").oninput = renderFilteredEquipmentRequests;
  bindEquipmentRequestRows();
}
...
function equipmentRequestPdiWo(row) {
  return String(row.notes || "").match(/PDI work order (WO-\d+)/i)?.[1] || "";
}

function equipmentRequestPdiWoBadge(row) {
  const wo = equipmentRequestPdiWo(row);
  return wo ? `<button class="rowbtn" type="button" data-equipment-request-open-wo="${esc(wo)}">${esc(wo)}</button>` : "";
}
...
function equipmentRequestCards(rows) {
  return rows.length ? rows.map((row) => `<article class="request-card" data-equipment-request-card="${esc(row.request_no)}">
    ...
    <div class="request-card-main">${esc(row.asset_tag || "")} ${esc(row.asset_name || "")}</div>
    <div class="request-card-meta">Needed ${esc(row.from_date || "")} to ${esc(row.to_date || "")}</div>
    <div class="request-card-meta">Requested by ${esc(row.requested_by || "")}${row.po_no ? ` | PO ${esc(row.po_no)}` : ""}</div>
...
function bindEquipmentRequestRows() {
  document.querySelectorAll("[data-equipment-request-edit]").forEach((b) => b.onclick = () => openEquipmentRequestModal(currentRows.find((row) => row.request_no === b.dataset.equipmentRequestEdit)));
...
async function openEquipmentRequestModal(row = null) {
  ...
  const assetValue = row?.asset_tag ? assetOptionLabel((productMeta.assets || []).find((asset) => asset.asset_tag === row.asset_tag) || { asset_tag: row.asset_tag, name: row.asset_name }) : "";
  ...
  $("modalBody").innerHTML = `
    <div class="form-grid equipment-request-form">
      ...
      <div class="field wide"><label>Equipment search</label><input class="suggest-input" list="workOrderAssetOptions" data-equipment-request-field="asset_lookup" value="${esc(assetValue)}" placeholder="Search asset tag, description, make, model, serial, plate, location" autocomplete="off">${assetOptionsDatalist()}</div>
      ...
      <div class="field"><label>Approved by</label><input data-equipment-request-field="approved_by" value="${esc(row?.approved_by || "")}"></div>
      <div class="field"><label>Status</label><input class="suggest-input" list="equipmentRequestStatusOptions" data-equipment-request-field="status" value="${esc(row?.status || "Requested")}" autocomplete="off"><datalist id="equipmentRequestStatusOptions"><option value="Requested"></option><option value="Approved"></option><option value="Approved - PDI Created"></option><option value="Denied"></option><option value="Issued"></option><option value="Returned"></option><option value="Cancelled"></option></datalist></div>
...
async function saveEquipmentRequestModal() {
  ...
  const asset = resolveAssetLookup(read("asset_lookup"));
  const fromDate = read("from_date");
  const toDate = read("to_date");
  if (!asset) return alert("Choose an equipment asset from the search list.");
  ...
  try {
    const query = editing
      ? supabase.from("equipment_requests").update(record).eq("request_no", editing.request_no)
      : supabase.from("equipment_requests").insert(record);
    const { error } = await query;
    if (error) throw error;
    if (!editing) await incrementSequence("equipment_request");
    await supabase.from("assets").update({ requested_by: record.requested_by, approved_by: record.approved_by }).eq("asset_tag", record.asset_tag);
    closeModal();
...
async function approveEquipmentRequest(requestNo) {
  ...
  if (!request) return;
  if (!confirm(`Approve ${requestNo} and create a Pre Delivery Inspection work order?`)) return;
  try {
    if (!productMeta.assets?.length) productMeta.assets = await getAll("assets");
    const asset = (productMeta.assets || []).find((row) => row.id === request.asset_id || row.asset_tag === request.asset_tag) || { asset_tag: request.asset_tag, name: request.asset_name };
    if (!asset?.asset_tag) throw new Error("This request has no valid asset. Edit the request and choose an equipment asset first.");
    const workOrders = await getAll("work_orders");
    const existing = workOrders.find((wo) => String(wo.notes || "").includes(`equipment request ${requestNo}`) || String(wo.description || "").includes(requestNo));
    const approver = profile?.full_name || profile?.username || session?.user?.email || "Approved";
    let woNo = existing?.wo_no || equipmentRequestPdiWo(request);
    if (!existing) {
      woNo = await nextRefPreview("wo", "WO-", "work_orders", "wo_no");
      const description = [
        `Pre Delivery Inspection for equipment request ${requestNo}.`,
        request.asset_tag ? `Asset: ${request.asset_tag} ${request.asset_name || asset.name || ""}.` : "",
      ...
      const saved = await upsertOne("work_orders", {
        wo_no: woNo,
        wo_date: localToday(),
        asset_id: asset.id || request.asset_id || null,
        asset_tag: asset.asset_tag,
        ...
        next_due_hours: 0,
        invoice_no: null,
        notes: `Auto-created from equipment request ${requestNo}. PDI work order ${woNo}.`,
      }, "wo_no");
      await upsertMany("work_order_issues", [{
        ...
        work_notes: `Created from equipment request ${requestNo}.`,
      }], "id");
      await incrementSequence("wo");
    }
    const notes = appendDatedNote(request.notes, `${approver} approved request. PDI work order ${woNo} created.`);
    const { error } = await supabase.from("equipment_requests").update({
      status: "Approved - PDI Created",
    ...
    await supabase.from("assets").update({
      ...
      approved_by: approver,
      status: woNo,
      notes: appendDatedNote(asset.notes, `Equipment request ${requestNo} approved; PDI work order ${woNo} created.`),
...
function assetRowHtml(asset, columns = assetVisibleColumns()) {
  return `<tr data-asset-row="${esc(asset.asset_tag || "")}">
    ...
    <td class="asset-col-actions"><div class="rowactions"><button class="rowbtn" type="button" data-asset-loc="${esc(asset.asset_tag)}">Loc</button><button class="rowbtn" type="button" data-asset-edit="${esc(asset.asset_tag)}">Edit</button><button class="rowbtn danger" type="button" data-asset-delete="${esc(asset.asset_tag)}">Delete</button></div></td>
  </tr>`;
}
...
async function openAssetModal(asset = null) {
  ...
  const qrPreviewUrl = assetQrUrl({ asset_tag: assetTag, vin_serial: asset?.vin_serial || "" });
...
async function saveAssetModal() {
  ...
  try {
    ...
    record.qr_update_url = assetQrUrl(record);
...
function assetQrUrl(asset = {}) {
  ...
  return `${location.origin}${location.pathname}#asset=${assetTag}${serial ? `&serial=${serial}` : ""}`;
}
...
async function importProductTemplateFile() {
  ...
  try {
    const rows = await readProductTemplateRows(file);
    const products = rows.map(productTemplateRowToDbProduct).filter((row) => row.sku && row.name);
    if (!products.length) throw new Error("No valid product rows were found. SKU and Product Name are required.");
...
function productTemplateRowToDbProduct(row) {
  const get = (...keys) => keys.map((key) => row[key]).find((value) => String(value ?? "").trim() !== "") ?? "";
  const sku = String(get("sku", "product_sku", "item_code")).trim();
  ...
  return {
    sku,
    name: String(get("product_name", "product", "name", "item")).trim(),
...
function assetImportRow(a) {
  return {
    ...
    qr_update_url: a.qrUpdateUrl || assetQrUrl({ asset_tag: a.assetTag, vin_serial: a.vinSerial }),
...
async function importLocalState(local) {
  ...
  await upsertMany("equipment_requests", (local.equipmentRequests || []).map((r, i) => {
    const asset = assetByTag.get(r.assetTag || r.asset_tag);
    return {
      request_no: r.requestNo || r.request_no || `ER-${1001 + i}`,
      request_date: r.requestDate || r.request_date || today(),
      asset_id: asset?.id || null,
      asset_tag: r.assetTag || r.asset_tag || null,
      asset_name: r.assetName || r.asset_name || asset?.name || null,

...
const tableMap = {
  ...
  truckingrequests: { table: "trucking_requests", key: "request_no", title: "Move Requests", sub: "Request trucking moves with multiple equipment lines and move descriptions before dispatch scheduling.", heads: ["request_no", "request_date", "customer", "project", "jobsite", "requested_by", "requestor_email", "needed_date", "notes"], labels: ["Request #", "Date", "Customer", "Project", "Jobsite", "Requested By", "Email", "Needed Date", "Move Description"] },
...
async function loadTruckingLookups() {
  ...
  productMeta.truckingJobsites = [
    ...
    ...(requests || []).map((row) => row.jobsite || row.project),
  ].filter(Boolean);
...
function truckingDatalists() {
  ...
  return `
    ...
    <datalist id="truckServiceOptions">${rates.map((rate) => `<option value="${esc(rate.service)}"></option>`).join("")}</datalist>
  ...
  if (isTruckingRequestorUser()) {
    return `<div class="truck-mobile-list">${filtered.map((row) => `
      <article class="truck-mobile-card">
        ...
        <p>${esc(row.customer || "No customer")} | ${esc(row.jobsite || row.project || "No jobsite")}</p>
        <small>${esc(row.request_date || "")}${row.notes ? ` | ${esc(row.notes)}` : ""}</small>
      </article>`).join("") || `<div class="empty">No move requests yet.</div>`}</div>`;
  }
  return truckingSimpleTable(filtered, ["request_no", "request_date", "customer", "project", "jobsite", "requested_by", "requestor_email", "needed_date", "line_count", "notes"], {
    labels: ["Request #", "Date", "Customer", "Project", "Jobsite", "Requested By", "Email", "Needed Date", "Lines", "Move Description"],
...
async function openTruckingRequestForm(requestNo = "") {
  ...
  const number = existing?.request_no || truckingNextNo("TRQ", productMeta.truckingRequests || [], "request_no", 1000);
  ...
  modalBody.innerHTML = `
    <div class="form-grid">
      ...
      <label>Date<input type="date" name="request_date" value="${esc(existing?.request_date || truckingToday())}"></label>
      <label>Customer<input name="customer" list="truckCustomerOptions" value="${esc(existing?.customer || "")}" placeholder="Customer"></label>
      <label>Project<input name="project" value="${esc(existing?.project || "")}" placeholder="Project"></label>
      <label>Jobsite<input name="jobsite" list="truckJobsiteOptions" value="${esc(existing?.jobsite || "")}" placeholder="Jobsite"></label>
      <label>Needed date<input type="date" name="needed_date" value="${esc(existing?.needed_date || "")}"></label>
    ...
    <div class="truck-lines" id="truckRequestLines">
      <div class="mini-grid truck-line-head"><strong>Equipment</strong><strong>Service</strong><strong>Origin</strong><strong>Destination</strong><strong>Notes</strong></div>
...
function truckingRequestLineHtml(line) {
  return `<div class="mini-grid truck-request-line">
    ...
    <input list="truckServiceOptions" class="suggest-input" name="line_service" value="${esc(line.service || "")}" placeholder="Service">
    <input name="line_origin" value="${esc(line.origin || "")}" placeholder="Origin">
    <input name="line_destination" value="${esc(line.destination || "")}" placeholder="Destination">
    <input name="line_notes" value="${esc(line.notes || "")}" placeholder="Notes">
  </div>`;
}

async function saveTruckingRequest(previousNo = "") {
  const values = Object.fromEntries([...modalBody.querySelectorAll("[name]")].filter((el) => !el.name.startsWith("line_")).map((el) => [el.name, el.value]));
  const existingRequest = previousNo ? (await getAll("trucking_requests")).find((row) => row.request_no === previousNo) : null;
  values.request_no = previousNo || values.request_no || nextRef("TRQ");
  ...
  const lines = [...modalBody.querySelectorAll(".truck-request-line")].map((row, index) => {
    ...
    return {
      ...
      service: row.querySelector("[name=line_service]")?.value.trim() || "",
      origin: row.querySelector("[name=line_origin]")?.value.trim() || "",
      destination: row.querySelector("[name=line_destination]")?.value.trim() || "",
  ...
  if (!values.jobsite) return alert("Jobsite is required.");

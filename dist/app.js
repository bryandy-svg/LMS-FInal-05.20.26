...
const tableMap = {
  ...
  trucking: { table: "trucking_moves", key: "ticket_no", title: "Trucking Scheduler", sub: "Dispatcher schedules, driver mobile moves, tickets, and billing support without accounting entries.", heads: ["ticket_no", "move_date", "customer", "project", "jobsite", "driver_name", "trip_type", "service", "equipment_label", "origin", "destination", "container_no", "start_time", "end_time", "amount", "status"], labels: ["Ticket #", "Date", "Customer", "Project", "Jobsite", "Driver", "Trip Type", "Service", "Equipment", "Origin", "Destination", "Container #", "Start", "End", "Amount", "Status"] },
...
async function renderTruckingRequestsView() {
  ...
  $("content").innerHTML = `
    ...
    <section class="panel">
      <div class="panel-head">
        <div class="panel-title"><strong>${isTruckingRequestorUser() ? "My Move Requests" : "Move Requests"}</strong><span>${esc(currentRows.length)} request${currentRows.length === 1 ? "" : "s"} shown.</span></div>
      ...
      <div id="truckRequestHost">${truckingRequestTable(currentRows)}</div>
    </section>
    ${truckingDatalists()}`;
...
function renderFilteredTruckingRequests() {
  $("truckRequestHost").innerHTML = truckingRequestTable(currentRows || []);
}
...
async function saveTruckingRequest(previousNo = "") {
  ...
  const lines = [...modalBody.querySelectorAll(".truck-request-line")].map((row, index) => {
    ...
    return {
      ...
      notes: [row.querySelector("[name=line_notes]")?.value.trim() || "", values.tipping_fee ? `Tipping fee: ${values.tipping_fee}` : ""].filter(Boolean).join(" | "),
      status: "Requested",
    };
  }).filter((line) => line.equipment_label || line.service || line.origin || line.destination);
...
function truckingMoveTable(rows) {
  ...
  return truckingSimpleTable(filtered, ["ticket_no", "move_date", "customer", "project", "jobsite", "driver_name", "trip_type", "service", "equipment_label", "origin", "destination", "container_no", "start_time", "end_time", "amount", "status"], {
    labels: ["Ticket #", "Date", "Customer", "Project", "Jobsite", "Driver", "Trip Type", "Service", "Equipment", "Origin", "Destination", "Container #", "Start", "End", "Amount", "Status"],
    format: { amount: (value) => money(value || 0), status: (value) => badge(value || "Scheduled") },
...
async function openTruckingDispatchFromRequest(requestNo) {
  ...
  modalBody.innerHTML = `
    <div class="form-grid">
      <label>Driver<input name="driver_name" list="truckDriverOptions" placeholder="Search driver"></label>
      <label>Move date<input type="date" name="move_date" value="${esc(truckingToday())}"></label>
    </div>
    <div class="truck-lines">
      <div class="mini-grid truck-line-head"><strong>Include</strong><strong>Requested Item</strong><strong>Service</strong><strong>Actual Truck / Equipment</strong><strong>Origin</strong><strong>Destination</strong></div>
      ${reqLines.map((line) => `<div class="mini-grid truck-dispatch-line" data-line="${esc(line.line_no)}">
        <input type="checkbox" checked>
        <span>${esc(line.equipment_label || "")}</span>
...
async function saveTruckingDispatch(req, reqLines) {
  ...
  for (const row of [...modalBody.querySelectorAll(".truck-dispatch-line")]) {
    ...
    moveRows.push({
      ...
      amount: Number(rate.rate || 0) * volumeMultiplier * tripMultiplier,
      origin: inputs[3]?.value.trim() || original.origin || "",
      destination: inputs[4]?.value.trim() || original.destination || "",
...
async function saveTruckingMove() {
  ...
  if (rate) {
    ...
    payload.amount = Number(rate.rate || 0) * (payload.trip_type === "Round Trip" && !/hour/i.test(String(rate.rate_type || "")) ? 2 : 1);
  }
...
async function printTruckingTicket(ticketNo) {
  ...
  const html = `<!doctype html><html><head><title>Ticket ${esc(row.ticket_no)}</title><style>
    body{font-family:Arial,Helvetica,sans-serif;color:#000;margin:0;padding:10px}.ticket{max-width:1050px;margin:auto;border:2px solid #111}.ticket-brand{padding:10px;border-bottom:2px solid #111}.ticket-brand img{display:block;width:100%;height:auto}.head{display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:8px;border-bottom:2px solid #111}.red{color:#d00}.checkrow{display:grid;grid-template-columns:repeat(6,1fr);gap:16px;padding:8px}.boxgrid{display:grid;grid-template-columns:1.15fr 1fr;border-top:2px solid #111}.cell{min-height:38px;border-bottom:1px solid #111;padding:4px 6px}.right{border-left:1px solid #111}.label{font-weight:900}.fill{background:#eaf1fb;font-style:italic;font-size:18px}.sign{height:80px}.sign img{display:block;max-width:100%;max-height:76px;object-fit:contain}.small{font-size:12px}.sigline{border-top:1px solid #111;margin-top:28px;padding-top:3px}@media print{@page{size:Letter landscape;margin:.25in}body{padding:0}.ticket{max-width:none}}</style></head><body>
    <div class="ticket">

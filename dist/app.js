...
async function renderTruckingTicketsView() {
  ...
  const unbilledCount = currentRows.filter((row) => String(row.billing_status || "Unbilled").toLowerCase() !== "billed").length;
  const billedCount = currentRows.filter((row) => String(row.billing_status || "").toLowerCase() === "billed").length;
  $("content").innerHTML = `
    <div class="toolbar"><input class="searchbox" id="truckingTicketSearch" placeholder="Search ticket, driver, customer, equipment, origin, destination"></div>
    <div class="tabbar trucking-request-tabs">
  ...
  const render = () => {
    const tabRows = currentRows.filter((row) => truckingTicketBillingTab === "billed"
      ? String(row.billing_status || "").toLowerCase() === "billed"
      : String(row.billing_status || "Unbilled").toLowerCase() !== "billed");
    const filtered = tabRows.filter((row) => truckingRowMatchesSearch(row, $("truckingTicketSearch")?.value || ""));
    $("truckingBillingSummaryHost").innerHTML = truckingCustomerBillingSummaryTable(filtered);
    document.querySelectorAll("[data-customer-billing-summary]").forEach((button) => {
      button.onclick = () => openTruckingCustomerBillingSummary(button.dataset.customerBillingSummary || "");
    });
    document.querySelectorAll("[data-mark-customer-billed]").forEach((button) => {
      button.onclick = () => markTruckingCustomerBilled(button.dataset.markCustomerBilled || "");
    });
    $("truckingTicketHost").innerHTML = truckingSimpleTable(filtered, ["ticket_no", "move_date", "customer", "driver_name", "equipment_label", "origin", "destination", "service", "start_time", "end_time", "move_description", "amount", "discount_amount", "amount_to_bill", "billing_status"], {
      labels: ["Ticket #", "Dispatch Date", "Customer", "Driver", "Actual Equipment", "Origin", "Destination", "Service", "Start Time", "End Time", "Move Description", "Original Charge", "Discount", "Amount to Bill", "Billing Status"],
      format: {
...
function truckingCustomerBillingSummaryTable(rows) {
  ...
  for (const row of rows) {
    ...
    const group = groups.get(key) || { customer, tickets: [], original: 0, discount: 0, billed: 0 };
  ...
  return truckingSimpleTable(summary, ["customer", "ticket_count", "original", "discount", "billed"], {
    labels: ["Customer", "Tickets", "Original Charges", "Discounts", "Amount to Be Billed"],
    format: {
    ...
    actions: (row) => `<button class="primary" data-customer-billing-summary="${esc(row.customer)}">${isTruckingDriverUser() ? "Combined PDF" : "Edit Discounts / Combined PDF"}</button>${truckingTicketBillingTab === "unbilled" && !isTruckingDriverUser() ? `<button data-mark-customer-billed="${esc(row.customer)}">Mark Billed</button>` : ""}`,
    empty: "No finalized tickets available for billing.",
  });
}

async function openTruckingCustomerBillingSummary(customer) {
  const rows = currentRows.filter((row) =>
...
async function markTruckingCustomerBilled(customer) {
  ...
  if (!ticketNos.length) return alert("No unbilled tickets found for this customer.");
  if (!confirm(`Mark ${ticketNos.length} ticket${ticketNos.length === 1 ? "" : "s"} for ${customer} as billed?`)) return;
  const { error } = await supabase.from("trucking_moves").update({
  ...
  const ticketPages = rows.map((row) => `
    <section class="ticket-page"><img class="brand" src="${esc(headerUrl)}" alt="Landscape Management Systems, Inc.">
      ...
      <div class="ticket-grid">
        ...
        <div><b>Driver</b><span>${esc(row.driver_name || "")}</span></div><div><b>Actual Equipment</b><span>${esc(row.equipment_label || "")}</span></div>
        <div><b>Start Time</b><span>${esc(row.start_time || "")}</span></div><div><b>End Time</b><span>${esc(row.end_time || "")}</span></div>
        <div class="wide"><b>Description of Work</b><span>${esc(row.move_description || row.notes || "")}</span></div>
...
async function printTruckingTicket(ticketNo) {
  ...
  const html = `<!doctype html><html><head><title>Ticket ${esc(row.ticket_no)}</title><style>
    ...
    <div class="ticket">
      ...
      <div class="boxgrid">
        <div><div class="cell label">Date:</div><div class="cell fill">${esc(row.move_date || "")}</div><div class="cell label">Origin:</div><div class="cell fill">${esc(row.origin || "")}</div><div class="cell label">Destination: &nbsp;&nbsp;&nbsp; Project:</div><div class="cell fill">${esc(row.destination || "")} &nbsp;&nbsp;&nbsp; ${esc(row.project || row.jobsite || "")}</div><div class="cell label">Actual Equipment:</div><div class="cell fill">${esc(row.equipment_label || "")}</div><div class="cell label">Description of Work:</div><div class="cell fill">${esc(row.move_description || row.notes || "")}</div><div class="cell label">Customer Print:</div><div class="cell fill">${esc(row.customer_print || row.customer || "")}</div><div class="cell label">Customer Signature: <span style="float:right">Date:</span></div><div class="cell sign">${signatureHtml(row.customer_signature)}</div></div>

...
function truckingMoveTable(rows) {
  ...
  return truckingSimpleTable(filtered, ["ticket_no", "move_date", "customer", "project", "jobsite", "driver_name", "trip_type", "service", "equipment_label", "origin", "destination", "container_no", "start_time", "end_time", "amount", "discount_amount", "amount_to_bill", "status"], {
    ...
    actions: (row) => `<button onclick="printTruckingTicket('${esc(row.ticket_no)}')">PDF</button><button onclick="openTruckingMoveForm('${esc(row.ticket_no)}')">Edit</button>`,
  });
}
...
function assignedDriverCards(rows) {
  ...
  return `<div class="truck-mobile-list">${filtered.map((row) => `
    <article class="truck-mobile-card">
      ...
      <p>Actual equipment: ${esc(row.equipment_label || "")}</p>
...
async function openAssignedDriverTask(ticketNo) {
  ...
  modalBody.innerHTML = `
    <div class="assigned-task-summary">
      ...
      <div><strong>Actual equipment</strong><span>${esc(row.equipment_label || "")}</span></div>
...
async function openTruckingMoveForm(ticketNo = "") {
  await loadTruckingLookups();
  const existing = ticketNo ? (await getAll("trucking_moves")).find((row) => row.ticket_no === ticketNo) : null;

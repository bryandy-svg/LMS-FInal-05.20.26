...
function rowsFromDetectedHeader(matrix) {
  ...
  const headerIndex = rows.findIndex((row) => {
    const headers = row.map(normalizeTemplateHeader);
    const hasAccount = headers.includes("account") || headers.includes("account_code");
    const hasAmount = headers.includes("debit") || headers.includes("credit") || headers.includes("debit_balance") || headers.includes("credit_balance");
    return hasAccount && hasAmount;
  });
  if (headerIndex < 0) return [];
  const headers = rows[headerIndex].map(normalizeTemplateHeader);

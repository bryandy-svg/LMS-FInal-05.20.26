...
function beginningDetailHeaderCandidates(preferredSheet = "") {
  const common = ["name", "payee", "invoice #", "invoice no", "invoice number", "invoice date", "due date", "amount", "invoice amount", "balance", "balance due", "open balance", "debit", "credit", "po #", "po no", "work order #", "jobsite", "equipment", "description of parts"];
...
function beginningDetailAmount(row, side = "debit") {
  ...
  const direct = nullableNumber(get("amount", "invoice_amount", "balance", "open_balance", "balance_due", "remaining_balance", "open_invoice_amount"));
  ...
  return Math.abs(debit != null ? debit : credit || 0);
}

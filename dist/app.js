...
async function loadFuelLookups() {
  const [fleet, outside, jobsites] = await Promise.all([
    getAll("assets").catch(() => []),
    getAll("outside_customer_fleet").catch(() => []),
    getAll("fuel_jobsites").catch(() => []),
  ]);
  productMeta.fuelJobsites = jobsites || [];
...
function fuelReportTable(rows) {
  if (!rows.length) return `<div class="empty">No fuel logs yet.</div>`;
  return `
    <div class="tablewrap">
      <table>
        <thead><tr>
          ${["Report #", "Date", "Jobsite", "Driver", "Source", "Asset #", "Equipment", "Time In", "Time Out", "Gallons", "Mileage / Hours", "Received By", "Signature", "Status"].map((h) => `<th>${esc(h)}</th>`).join("")}
        </tr></thead>
        <tbody>${rows.map((row) => `
          <tr>

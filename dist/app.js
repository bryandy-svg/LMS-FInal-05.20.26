...
async function renderTruckingTicketsView() {
  ...
  const render = () => {
    const filtered = currentRows.filter((row) => truckingRowMatchesSearch(row, $("truckingTicketSearch")?.value || ""));
    ...
    document.querySelectorAll("[data-customer-billing-summary]").forEach((button) => {
      button.onclick = () => printTruckingCustomerBillingSummary(button.dataset.customerBillingSummary || "");
    });
    $("truckingTicketHost").innerHTML = truckingSimpleTable(filtered, ["ticket_no", "move_date", "customer", "driver_name", "equipment_label", "origin", "destination", "service", "finalized_at", "status"], {
      labels: ["Ticket #", "Dispatch Date", "Customer", "Driver", "Actual Equipment", "Origin", "Destination", "Service", "Finalized", "Status"],
      format: { status: (value) => badge(value) },
...
function truckingCustomerBillingSummaryTable(rows) {
  ...
  return truckingSimpleTable(summary, ["customer", "ticket_count", "original", "discount", "billed"], {
    ...
    actions: (row) => `<button class="primary" data-customer-billing-summary="${esc(row.customer)}">View Billing Summary / PDF</button>`,
    empty: "No finalized tickets available for billing.",
  });
}

async function printTruckingCustomerBillingSummary(customer) {
  ...
  const html = `<!doctype html><html><head><title>Billing Summary - ${esc(customer)}</title><style>
    @page{size:Letter landscape;margin:.35in}*{box-sizing:border-box}body{font-family:Arial,Helvetica,sans-serif;color:#111;margin:0}.sheet{width:100%;page-break-inside:avoid}.brand{width:100%;max-height:1.05in;object-fit:contain;display:block;margin-bottom:8px}.title{display:flex;justify-content:space-between;align-items:flex-end;border-bottom:2px solid #111;padding-bottom:8px;margin-bottom:10px}.title h1{font-size:22px;margin:0}.title div{text-align:right}table{width:100%;border-collapse:collapse;font-size:10px}th,td{border:1px solid #777;padding:5px 6px;text-align:left}th{background:#e9eef1;text-transform:uppercase;font-size:9px}.num{text-align:right}.totals{width:44%;margin:12px 0 0 auto}.totals div{display:flex;justify-content:space-between;border-bottom:1px solid #aaa;padding:6px}.totals .due{font-size:16px;font-weight:900;border:2px solid #111}.muted{color:#555}</style></head><body>
    <main class="sheet"><img class="brand" src="${esc(headerUrl)}" alt="Landscape Management Systems, Inc.">
      ...
      <table><thead><tr><th>Ticket #</th><th>Dispatch Date</th><th>Service</th><th>Origin</th><th>Destination</th><th>Driver</th><th>Actual Equipment</th><th>Payment</th><th>PO #</th><th class="num">Original</th><th class="num">Discount</th><th class="num">Amount to Bill</th></tr></thead>
      <tbody>${rows.map((row) => `<tr><td>${esc(row.ticket_no)}</td><td>${esc(row.move_date || "")}</td><td>${esc(row.service || "")}</td><td>${esc(row.origin || "")}</td><td>${esc(row.destination || "")}</td><td>${esc(row.driver_name || "")}</td><td>${esc(row.equipment_label || "")}</td><td>${esc(row.payment_mode || "")}</td><td>${esc(row.po_no || "")}</td><td class="num">${money(row.amount || 0)}</td><td class="num">${money(row.discount_amount || 0)}</td><td class="num"><strong>${money(row.amount_to_bill ?? row.amount ?? 0)}</strong></td></tr>`).join("")}</tbody></table>
...
async function openTruckingMoveForm(ticketNo = "") {
  ...
  modalBody.innerHTML = `
    ...
    <label>Notes<textarea name="notes">${esc(existing?.notes || "")}</textarea></label>
    ${truckingDatalists()}`;

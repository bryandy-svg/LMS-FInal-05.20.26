...
const tableMap = {
  ...
  fuel: { table: "fuel_logs", key: "report_no", title: "Fuel Portal", sub: "Daily fuel refill reports by jobsite, driver, equipment, gallons, time, and receiving signature.", heads: ["report_no", "fuel_date", "jobsite", "driver_name", "asset_source", "asset_tag", "equipment_name", "time_in", "time_out", "gallons", "mileage_hours", "receiver_name", "status"], labels: ["Report #", "Date", "Jobsite", "Driver", "Source", "Asset #", "Equipment", "Time In", "Time Out", "Gallons", "Mileage / Hours", "Received By", "Status"] },
...
async function updateFuelAssetLocationsFromFuel(lines, jobsite, fuelDate) {
  ...
  await supabase.from("locations").upsert({ name: location }, { onConflict: "name" }).catch(() => {});
  ...
  const updateOne = async (line) => {
    ...
    for (const attempt of attempts) {
      ...
      if (table === "assets") {
        await supabase.from(table).update({ last_update_date: updateDate, scanned_date: stampedAt }).eq(attempt.field, attempt.value).catch(() => {});
      }

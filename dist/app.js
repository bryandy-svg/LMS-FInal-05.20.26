...
async function saveTruckingRequest(previousNo = "") {
  ...
  if (values.service_needed.toLowerCase() !== "roll off service") values.tipping_fee = "";
  ...
  const lines = [...modalBody.querySelectorAll(".truck-request-line")].map((row) => {
    ...
    return {
      ...
      notes: [lineNotes, values.tipping_fee ? `Tipping fee: ${values.tipping_fee}` : ""].filter(Boolean).join(" | "),
      status: "Requested",
    };
...
async function saveTruckingDispatch(req, reqLines) {
  ...
  for (const row of [...modalBody.querySelectorAll(".truck-dispatch-line")]) {
    ...
    const originalCharge = Number(rate.rate || 0) * volumeMultiplier * tripMultiplier;

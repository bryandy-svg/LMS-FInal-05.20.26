...
try {
  ...
  console.error("Supabase connection setup failed", error);
}
...
async function boot() {
  ...
  if (!supabase) {
    ...
    $("sessionBox").innerHTML = "Database not connected";
    $("content").innerHTML = `
      <section class="panel">
        <div class="panel-head">
          <div class="panel-title">
            <strong>Database connection is missing</strong>
            <span>The app opened, but it does not have the Supabase URL/key for this test or production system.</span>
          </div>
          <div class="actions"><button id="retryConfigBtn">Refresh</button></div>
        </div>
        <div class="empty">
          In Vercel, open this project settings and make sure <strong>SUPABASE_URL</strong> and <strong>SUPABASE_ANON_KEY</strong> are filled for this exact project. Production should use the production Supabase values. Test should use the test Supabase values.
        </div>
      </section>`;
    $("retryConfigBtn").onclick = () => location.reload();
    return;
  }

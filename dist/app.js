...
async function openLandedCostInvoiceModal(sourcePo) {
  ...
  const sourceSet = new Set([sourcePo.po_no]);
  ...
  $("modalBody").innerHTML = `
    <div class="form-grid">
      ...
      ${productSelect("Allocation method", "allocation_method", ["By Value", "By Qty", "Manual"], "By Value")}
  ...
  document.querySelectorAll("[data-landed-amount]").forEach((input) => input.oninput = () => {
    ...
    if (/manual/i.test(method)) refreshLandedCostAllocationTotal();
  });
    ...
    try {
      const landedPo = {
        ...
        notes: [
          ...
          `Allocation method: ${record.allocation_method || "By Value"}`,
          ...allocations.map((row) => `Allocation: ${row.po_no} | ${Number(row.amount || 0).toFixed(2)} | ${record.allocation_method || "By Value"}`),
          record.notes || ""
        ].filter(Boolean).join("\n"),
      };
...
function landedCostAllocationRows(pos, selectedPoNos = new Set()) {
  const heads = ["Use", "PO", "Vendor", "Jobsite / Project", "Parts", "Estimated Landed", "Already AP", "Allocate"];
  ...
  }).join("")}</tbody><tfoot><tr><td colspan="7" class="num"><strong>Allocated total</strong></td><td id="landedAllocationTotal">${money(0)}</td></tr></tfoot></table></div>`;
}

function refreshLandedCostAllocationPreview() {
  const method = document.querySelector('[data-product-field="allocation_method"]')?.value || "By Value";
  if (/manual/i.test(method)) {
  ...
  const baseTotal = pos.reduce((sum, po) => sum + (/qty/i.test(method) ? po._allocationQty : po._allocationValue), 0);
  let allocatedSoFar = 0;
  pos.forEach((po, index) => {
    const input = rows.find((tr) => tr.dataset.landedPo === po.po_no)?.querySelector("[data-landed-amount]");
    if (!input) return;
    const base = /qty/i.test(method) ? po._allocationQty : po._allocationValue;
...
function collectLandedCostAllocations() {
  return [...document.querySelectorAll("[data-landed-po]")]
    ...
    .map((tr) => ({ po_no: tr.dataset.landedPo, amount: Number(tr.querySelector("[data-landed-amount]")?.value || 0) }))
...
async function openEquipmentRequestModal(row = null) {
  ...
  const [assets, locations] = await Promise.all([
    productMeta.assets?.length ? productMeta.assets : getAll("assets"),
    productMeta.locations?.length ? productMeta.locations : getAll("asset_locations").then((rows) => rows.map((row) => row.name || row.location || row.value).filter(Boolean)).catch(() => []),
  ]);
  productMeta.assets = assets || [];

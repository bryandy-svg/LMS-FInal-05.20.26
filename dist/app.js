...
const withTimeout = (promise, ms = 8000) => Promise.race([
  promise,
  new Promise((_, reject) => setTimeout(() => reject(new Error("This database request took too long. Please refresh and try again.")), ms)),
]);
...
async function boot() {
  ...
  try {
    ...
    if (session) await withTimeout(loadProfile(), 8000);
    if (session) withTimeout(loadAccountingCloseDate(), 5000).catch((error) => console.warn("Accounting close date skipped", error));
    renderNav();
    updateAuthView();
    if (publicScan) {
    ...
    $("content").innerHTML = `<section class="panel"><div class="panel-head"><div class="panel-title"><strong>Could not start the system</strong><span>The app opened, but one startup request stopped responding.</span></div><div class="actions"><button id="retryBootBtn">Retry</button></div></div><div class="empty">${esc(error.message || error)}</div></section>`;
...
async function renderDashboard() {
  ...
  $("content").innerHTML = `<div class="empty">Loading dashboard summary...</div>`;
  const [products, sales, po, assets, repairs, rentals, invoices, gl] = await Promise.all([
    getDashboardRows("products", "qty,cost,reorder_point", 2000),
    getDashboardRows("sales_orders", "status", 500),
    getDashboardRows("purchase_orders", "payment_status,match_status", 500),
    getDashboardRows("assets", "id", 500),
    getDashboardRows("work_orders", "status", 500),
    getDashboardRows("rentals", "status", 500),
    getDashboardRows("invoices", "status", 500),
    getDashboardRows("general_ledger", "debit,credit", 1000),
  ]);
...
async function getDashboardRows(table, columns = "*", limit = 500) {
  const { data, error } = await withTimeout(supabase.from(table).select(columns).limit(limit), 7000);
  if (error) {
...
async function openPurchaseOrderModal(po = null) {
  ...
  $("modalBody").innerHTML = `
    <div class="form-grid">
      ...
      ${productSelect("Landed cost allocation inside this PO", "landed_cost_method", ["By Value", "By Qty", "By Weight", "Manual"], po?.landed_cost_method || "By Value")}
    ...
    try {
      const landedPo = {
        ...
        notes: [
          ...
          `Allocation method: ${record.custom_allocation_method || record.allocation_method || "By Value"}`,
          ...allocations.map((row) => `Allocation: ${row.po_no} | ${Number(row.amount || 0).toFixed(2)} | ${record.custom_allocation_method || record.allocation_method || "By Value"} | Weight ${Number(row.weight || 0)} | PO lines ${row.line_method || "By Value"}`),
          record.notes || ""
        ].filter(Boolean).join("\n"),
      };
...
function landedCostAllocationRows(pos, selectedPoNos = new Set()) {
  const heads = ["Use", "PO", "Vendor", "Jobsite / Project", "Parts", "Estimated Landed", "Already AP", "Shipment Weight", "Allocate", "Distribute Inside PO"];
  return `<div class="table-wrap"><table class="mini-table landed-allocation-table"><thead><tr>${heads.map((h) => `<th>${esc(h)}</th>`).join("")}</tr></thead><tbody>${pos.map((po) => {
    ...
    return `<tr data-landed-po="${esc(po.po_no)}" data-landed-value="${esc(poPartsSubtotal(po))}" data-landed-qty="${esc((po._lines || []).reduce((sum, line) => sum + Number(line.qty || 0), 0))}">
      ...
      <td><input type="number" step="0.01" data-landed-weight value="${esc(po.shipment_weight || "")}" placeholder="lb/kg"></td>
      <td><input type="number" step="0.01" data-landed-amount value="${esc(checked ? remaining : 0)}"></td>
      <td><select data-landed-line-method>${["By Value", "By Qty", "By Weight", "Manual"].map((method) => `<option ${String(po.landed_cost_method || "By Value") === method ? "selected" : ""}>${esc(method)}</option>`).join("")}</select></td>
    </tr>`;
  }).join("")}</tbody><tfoot><tr><td colspan="9" class="num"><strong>Allocated total</strong></td><td id="landedAllocationTotal">${money(0)}</td></tr></tfoot></table></div>`;
}
...
function collectLandedCostAllocations() {
  return [...document.querySelectorAll("[data-landed-po]")]
    ...
    .map((tr) => ({
      po_no: tr.dataset.landedPo,
      amount: Number(tr.querySelector("[data-landed-amount]")?.value || 0),
      weight: Number(tr.querySelector("[data-landed-weight]")?.value || 0),
      line_method: tr.querySelector("[data-landed-line-method]")?.value || "By Value",
    }))

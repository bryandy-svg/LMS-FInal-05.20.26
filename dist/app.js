...
const assetColumnDefs = [
  ["photo", "Photo"], ["new_qr", "New QR"], ["old_qr_code", "Old QR Code"], ["asset_tag", "Asset #"], ["description", "Description"], ["type", "Type"],
  ...
  ["last_update_date", "Last Update Date"], ["scanned_date", "Scanned Date"], ["reading", "Reading"], ["assigned_operator", "Operator"], ["status", "Status"],
];
const ASSET_COLUMN_STORAGE_KEY = "lms.assetColumns.v2";
...
function assetCellHtml(asset, key) {
  ...
  const oldQrCode = asset.old_qr_code || asset.old_qr || asset.legacy_qr_code || asset.qr_code || asset.qr_printed_tagged || "";
  const lastUpdateDate = asset.last_update_date || asset.last_updated || asset.updated_date || String(asset.updated_at || "").slice(0, 10);
  const cells = {
...
function exportAssetsWithQrCsv() {
  const heads = ["Photo URL", "New QR Picture", "New QR Update URL", "Old QR Code", "Asset #", "Description", "Type", "General Type", "Parent Asset", "Relationship", "Compatible With", "Make", "Model", "Color", "VIN / Serial", "Plate", "Location", "Last Update Date", "Scanned Date", "Odometer", "Engine Hours", "Operator", "Status"];
  const body = currentRows.map((a) => {
    const qrText = a.qr_update_url || assetQrUrl(a);
    const oldQrCode = a.old_qr_code || a.old_qr || a.legacy_qr_code || a.qr_code || a.qr_printed_tagged || "";
    const lastUpdateDate = a.last_update_date || a.last_updated || a.updated_date || String(a.updated_at || "").slice(0, 10);
    const values = [
      a.photo_url ? `<a href="${esc(a.photo_url)}">${esc(a.photo_url)}</a>` : "",
      `<img class="qr-export" src="${qrCodeUrl(qrText, 160)}" alt="QR">`,
      `<a href="${esc(qrText)}">${esc(qrText)}</a>`,
      oldQrCode,
      a.asset_tag,
      a.name,
      a.type,
    ...
    return `<tr>${values.map((value, index) => index === 1 || index < 3 ? `<td>${value || ""}</td>` : `<td>${esc(value || "")}</td>`).join("")}</tr>`;
  }).join("");
  const html = `<!doctype html><html><head><meta charset="utf-8"><style>
    ...
    .qr-export{width:72px;height:72px;object-fit:contain;display:block;margin:auto}
    tr{height:78px}

...
async function importContactMasterFile(view) {
  ...
  try {
    ...
    const batch = Date.now().toString().slice(-6);
    const records = rows.map((row, index) => contactMasterRowToDb(row, prefix, batch, index)).filter((row) => row.name);
    ...
    await upsertMany(view, records, "reference");
    input.value = "";
    alert(`${records.length} ${view === "vendors" ? "vendor" : "customer"} row${records.length === 1 ? "" : "s"} uploaded.`);
    await loadView(view);
  } catch (error) {
    input.value = "";
...
function contactMasterRowToDb(row, prefix, batch, index) {
  const get = (...keys) => keys.map((key) => row[key]).find((value) => String(value ?? "").trim() !== "") ?? "";
  return {
    reference: String(get("reference", "ref", "vendor_reference", "customer_reference")).trim() || `${prefix}${batch}-${index + 1}`,
    name: String(get("name", "vendor_name", "customer_name", "company_name")).trim(),

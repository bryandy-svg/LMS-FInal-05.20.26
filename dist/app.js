...
const assetColumnDefs = [
  ["photo", "Photo"], ["new_qr", "New QR"], ["old_qr_code", "Old QR Code"], ["asset", "Asset"], ["type", "Type"],
  ...
  ["last_update_date", "Last Update"], ["scanned_date", "Scanned Date"], ["reading", "Reading"], ["assigned_operator", "Operator"], ["status", "Status"],
];
...
function assetCellHtml(asset, key) {
  ...
  const cells = {
    ...
    old_qr_code: esc(asset.old_qr_code || ""),
    asset: `<strong>${esc(asset.asset_tag)}</strong><br>${esc(asset.name || "")}`,
    type: esc(asset.type || ""),
    ...
    last_update_date: esc(asset.last_update_date || ""),
...
function assetVisibleColumns() {
  const saved = JSON.parse(localStorage.getItem("lms.assetColumns") || "null");
  const keys = Array.isArray(saved) && saved.length ? saved : assetColumnDefs.map(([key]) => key);
...
function saveAssetColumnChoice() {
  ...
  localStorage.setItem("lms.assetColumns", JSON.stringify(keys.length ? keys : assetColumnDefs.map(([key]) => key)));
...
function exportAssetsWithQrCsv() {
  const heads = ["Photo URL", "New QR Picture", "New QR Update URL", "Old QR Code", "Asset", "Type", "General Type", "Name", "Parent Asset", "Relationship", "Compatible With", "Make", "Model", "Color", "VIN / Serial", "Plate", "Location", "Last Update Date", "Scanned Date", "Odometer", "Engine Hours", "Operator", "Status"];

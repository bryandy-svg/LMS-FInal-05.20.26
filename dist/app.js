...
function workOrderNarrativeEvents(wo) {
  ...
  (wo._parts || []).filter((part) => Number(part.accepted_qty || 0) > 0 || /accepted|released|shortage|reserved|requested/i.test(part.status || "")).forEach((part) => {
    const lines = [];
    const accepted = Number(part.accepted_qty || 0);
    const needed = Number(part.qty_needed || 0);
    lines.push(`Part: ${partDisplayName(part)} - needed ${needed}, accepted ${accepted}, status ${effectivePartStatus(part)}.`);
    if (part.issue) lines.push(`For issue: ${part.issue}`);
    if (part.notes) lines.push(`Notes: ${part.notes}`);
    events.push({ date: wo.wo_date || "", sort: `${wo.wo_date || ""}T18:00:00.000Z`, lines });
  });

...
const tableMap = {
  ...
  truckingrequests: { table: "trucking_requests", key: "request_no", title: "Move Requests", sub: "Request trucking moves with configurable service patterns, rates, locations, and equipment details.", heads: ["request_no", "request_date", "customer", "project_jobsite", "contact_no", "needed_date", "delivery_period", "trip_type", "service_needed", "service_subcategory", "service_size", "service_reference", "material_description", "unit_of_measurement", "weight_lb", "dimensions", "tipping_fee", "origin", "destination", "requested_by", "requestor_email", "notes"], labels: ["Request #", "Date", "Customer", "Project / Jobsite", "Contact #", "Requested Delivery", "AM / PM", "Trip Type", "Super Parent", "Move Type / Option", "Size / Capacity", "Reference #", "Material(s)", "Unit of Measurement", "Weight (lb)", "Dimensions", "Tipping Fee", "Origin", "Destination", "Requested By", "Email", "Move Description"] },
...
function truckingDatalists() {
  ...
  const standardServices = ["Equipment Move", "Container/Trailer Move", "High Deck Trailer", "Water Service", "Roll Off Service", "Pump Service"];
...
function truckingServiceSelectOptions(selected = "") {
  const standard = ["Equipment Move", "Container/Trailer Move", "High Deck Trailer", "Water Service", "Roll Off Service", "Pump Service"];
...
function updateTruckingPatternFields(root, service, selected = {}) {
  ...
  const needsHighDeckDetails = String(service || "").trim().toLowerCase() === "high deck trailer";
...
function truckingRequestTable(rows) {
  ...
  return truckingSimpleTable(filtered, ["request_no", "request_date", "customer", "project_jobsite", "contact_no", "needed_date", "delivery_period", "trip_type", "service_needed", "service_subcategory", "service_size", "service_reference", "material_description", "unit_of_measurement", "weight_lb", "dimensions", "tipping_fee", "origin", "destination", "requested_by", "line_count", "notes"], {
    labels: ["Request #", "Date", "Customer", "Project / Jobsite", "Contact #", "Requested Delivery", "AM / PM", "Trip Type", "Super Parent", "Move Type / Option", "Size / Capacity", "Reference #", "Material(s)", "Unit of Measurement", "Weight (lb)", "Dimensions", "Tipping Fee", "Origin", "Destination", "Requested By", "Lines", "Move Description"],
...
async function openTruckingRequestForm(requestNo = "") {
  ...
  modalBody.innerHTML = `
    ...
    <div class="form-grid">
      ...
      <label id="truckServiceSizeField" hidden>Size / capacity<select name="service_size"></select></label>
...
async function saveTruckingRequest(previousNo = "") {
  ...
  if (values.service_needed.toLowerCase() !== "high deck trailer") {
  ...
  const allowedServices = [...new Set([
    "Equipment Move", "Container/Trailer Move", "High Deck Trailer", "Water Service", "Roll Off Service", "Pump Service",
  ...
  if (values.service_needed.toLowerCase() === "high deck trailer") {
    if (!values.material_description) return alert("Material(s) is required for High Deck Trailer.");
    if (!values.unit_of_measurement) return alert("Unit of measurement is required for High Deck Trailer.");
    if (!Number(values.weight_lb || 0)) return alert("Weight (lb) is required for High Deck Trailer.");
    if (!values.dimensions) return alert("Dimensions are required for High Deck Trailer.");
  }
...
async function saveTruckingDispatch(req, reqLines) {
  ...
  for (const row of [...modalBody.querySelectorAll(".truck-dispatch-line")]) {
    ...
    moveRows.push({
      ...
      amount: Number(rate.rate || 0) * tripMultiplier,
      ...
      notes: [
        original.notes || "",
        req.service_needed === "High Deck Trailer" ? `Material: ${req.material_description}; UOM: ${req.unit_of_measurement}; Weight: ${req.weight_lb} lb; Dimensions: ${req.dimensions}` : "",
...
async function openTruckingRateForm(service = "") {
  ...
  const standardParents = ["Equipment Move", "Container/Trailer Move", "High Deck Trailer", "Water Service", "Roll Off Service", "Pump Service"];

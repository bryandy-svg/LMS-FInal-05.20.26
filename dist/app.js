...
const tableMap = {
  ...
  truckingrequests: { table: "trucking_requests", key: "request_no", title: "Move Requests", sub: "Request trucking moves with configurable service patterns, rates, locations, and equipment details.", heads: ["request_no", "request_date", "customer", "project_jobsite", "contact_no", "needed_date", "delivery_period", "trip_type", "service_needed", "service_subcategory", "service_size", "service_reference", "tipping_fee", "origin", "destination", "requested_by", "requestor_email", "notes"], labels: ["Request #", "Date", "Customer", "Project / Jobsite", "Contact #", "Requested Delivery", "AM / PM", "Trip Type", "Super Parent", "Move Type / Option", "Size / Capacity", "Reference #", "Tipping Fee", "Origin", "Destination", "Requested By", "Email", "Move Description"] },
...
function truckingRequestTable(rows) {
  ...
  return truckingSimpleTable(filtered, ["request_no", "request_date", "customer", "project_jobsite", "contact_no", "needed_date", "delivery_period", "trip_type", "service_needed", "service_subcategory", "service_size", "service_reference", "tipping_fee", "origin", "destination", "requested_by", "line_count", "notes"], {
    labels: ["Request #", "Date", "Customer", "Project / Jobsite", "Contact #", "Requested Delivery", "AM / PM", "Trip Type", "Super Parent", "Move Type / Option", "Size / Capacity", "Reference #", "Tipping Fee", "Origin", "Destination", "Requested By", "Lines", "Move Description"],
...
async function saveTruckingDispatch(req, reqLines) {
  ...
  for (const row of [...modalBody.querySelectorAll(".truck-dispatch-line")]) {
    ...
    moveRows.push({
      ...
      notes: [original.notes || "", `${tripType} charge: ${tripMultiplier}×`].filter(Boolean).join(" | "),
      status: "Scheduled",
    });
...
function truckingRatesTable(rows) {
  ...
  return truckingSimpleTable(sorted.filter((row) => truckingRowMatchesSearch(row, $("truckRateSearch")?.value || "")), ["parent_service", "subcategory", "size_capacity", "service", "reference_label", "requires_equipment", "rate", "rate_type", "additional", "category", "status"], {
    ...
    actions: (row) => `<button onclick="openTruckingRateForm('${esc(row.service)}')">Edit</button>`,
  });
}
...
async function openTruckingRateForm(service = "") {
  ...
  modalBody.innerHTML = `
    ...
    <label>Additional / notes<textarea name="additional">${esc(row?.additional || "")}</textarea></label>`;

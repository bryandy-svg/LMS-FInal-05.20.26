...
const modules = [
  ...
  { group: "Controls", items: [["controlcenter", "Control Center"], ["periods", "Period Close"], ["numbering", "Numbering Setup"], ["notifications", "Notifications"], ["approvals", "Approvals"], ["auditlog", "Audit Log"], ["documents", "Documents"]] },
...
const tableMap = {
  ...
  notifications: { table: "system_notifications", key: "id", title: "Notifications", sub: "Open reminders, exceptions, and review items across the system.", heads: ["created_at", "severity", "module", "title", "message", "record_ref", "assigned_to", "status", "resolved_at"], labels: ["Created", "Severity", "Module", "Title", "Message", "Reference", "Assigned To", "Status", "Resolved"] },
  approvals: { table: "approval_queue", key: "id", title: "Approvals", sub: "Requests waiting for manager or accounting review.", heads: ["created_at", "module", "record_ref", "approval_type", "requested_by", "assigned_to", "status", "notes"], labels: ["Created", "Module", "Reference", "Approval Type", "Requested By", "Assigned To", "Status", "Notes"] },
  auditlog: { table: "audit_log", key: "id", title: "Audit Log", sub: "Who changed what, when, and from which module.", heads: ["event_at", "event_type", "module", "record_ref", "user_name", "summary", "source"], labels: ["Date / Time", "Event", "Module", "Reference", "User", "Summary", "Source"], readOnly: true },
  documents: { table: "document_attachments", key: "id", title: "Documents", sub: "Attachments by module and reference, such as POs, invoices, photos, and signed documents.", heads: ["created_at", "module", "record_ref", "file_name", "file_type", "file_size", "uploaded_by", "notes", "file_url"], labels: ["Created", "Module", "Reference", "File Name", "Type", "Size", "Uploaded By", "Notes", "Link"] },
  periods: { table: "accounting_periods", key: "period_name", title: "Period Close", sub: "Close posting periods so prior AP, AR, GL, checks, receipts, sales, and inventory records cannot be changed without admin reopening.", heads: ["period_name", "start_date", "end_date", "closed_through_date", "status", "closed_by", "closed_at", "reopened_by", "reopened_at", "notes"], labels: ["Period", "Start", "End", "Closed Through", "Status", "Closed By", "Closed At", "Reopened By", "Reopened At", "Notes"] },
  numbering: { table: "numbering_sequences", key: "sequence_key", title: "Numbering Setup", sub: "Reference numbering for POs, goods receipts, work orders, invoices, check runs, equipment requests, and other controlled documents.", heads: ["sequence_key", "prefix", "next_number", "padding", "notes", "updated_at"], labels: ["Sequence Key", "Prefix", "Next Number", "Padding", "Notes", "Updated"] },
  controlcenter: { title: "Control Center", sub: "Audit, approvals, notifications, document attachments, period close, numbering, import review, master cleanup, and backup controls.", derived: true },
...
const TABLE_VIEW_STORAGE_KEY = "lms.tableSavedViews.v1";
...
function bindChrome() {
  ...
  document.querySelectorAll(".action-menu, .inline-action-menu").forEach((menu) => {
    menu.addEventListener("toggle", () => {
      if (!menu.open) return;
      document.querySelectorAll(".action-menu, .inline-action-menu").forEach((other) => {
        if (other !== menu) other.open = false;
      });
    });
  });
  document.addEventListener("click", (event) => {
    if (event.target.closest(".action-menu, .inline-action-menu")) return;
    document.querySelectorAll(".action-menu, .inline-action-menu").forEach((menu) => {
      menu.open = false;
    });
  });
...
function isAdminUser() {
  const role = String(profile?.role || "");
  return /admin|administrator|owner/i.test(role) || userModules().includes("all");
}
...
function updateAuthView() {
  ...
  document.querySelectorAll(".admin-menu").forEach((menu) => {
    menu.style.display = session && isAdminUser() ? "" : "none";
  });
...
async function loadView(view) {
  ...
  try {
    ...
    if (view === "controlcenter") return await renderControlCenterView();
...
async function renderDashboard() {
  ...
  $("content").innerHTML = `
    <section class="panel">
      <div class="panel-head"><div class="panel-title"><strong>Loading dashboard</strong><span>Opening the summary without waiting on full transaction history.</span></div></div>
      <div class="empty">Preparing current stock, sales, fleet, and accounting snapshots...</div>
    </section>`;
  const [products, sales, po, assets, repairs, rentals, invoices, gl] = await Promise.all([
    getDashboardRows("products", "*", 2500),
    getDashboardRows("sales_orders", "*", 1000),
    getDashboardRows("purchase_orders", "*", 1000),
    getDashboardRows("assets", "*", 1500),
    getDashboardRows("work_orders", "*", 1000),
    getDashboardRows("rentals", "*", 1000),
    getDashboardRows("invoices", "*", 1000),
    getDashboardRows("general_ledger", "posting_date,account,description,reference,debit,credit,source,status", 1500),
  ]);
  ...
  const unmatchedPo = po.filter((p) => /mismatch|awaiting|pending/i.test(`${p.match_status || ""} ${p.status || ""}`)).length;
  const readyPay = po.filter((p) => /ready/i.test(p.payment_status || "")).length;
  const unpaidInvoices = invoices.filter((i) => !/paid|void|reversed/i.test(i.status || "")).length;
  const readyClose = repairs.filter((r) => /ready to close/i.test(r.status || "")).length;
  const missingCustomerPo = sales.filter((s) => !s.customer_po && !/yes/i.test(s.manager_override || "")).length;
  const assetsNeedQr = assets.filter((a) => /yes|true|1/i.test(String(a.needs_qr_code_printed || ""))).length;
  const debit = gl.reduce((s, r) => s + Number(r.debit || 0), 0);
  const credit = gl.reduce((s, r) => s + Number(r.credit || 0), 0);
  const alerts = [
    { label: `${low} inventory item${low === 1 ? "" : "s"} at or below reorder point.`, view: "products", level: low ? "warn" : "good" },
    { label: `${unmatchedPo} PO/GR/invoice record${unmatchedPo === 1 ? "" : "s"} need matching review.`, view: "purchasing", level: unmatchedPo ? "warn" : "good" },
    { label: `${readyPay} payable${readyPay === 1 ? "" : "s"} ready for check run.`, view: "checkrun", level: readyPay ? "warn" : "good" },
    { label: `${unpaidInvoices} customer invoice${unpaidInvoices === 1 ? "" : "s"} still open.`, view: "invoices", level: unpaidInvoices ? "warn" : "good" },
    { label: `${readyClose} work order${readyClose === 1 ? "" : "s"} ready to close.`, view: "repairs", level: readyClose ? "warn" : "good" },
    { label: `${missingCustomerPo} sales order${missingCustomerPo === 1 ? "" : "s"} missing customer PO or manager override.`, view: "orders", level: missingCustomerPo ? "bad" : "good" },
    { label: `${assetsNeedQr} asset${assetsNeedQr === 1 ? "" : "s"} need QR code printed.`, view: "assets", level: assetsNeedQr ? "warn" : "good" },
  ];
  ...
  $("content").innerHTML = `
    ...
    <section class="panel workflow-panel">
      <div class="panel-head"><div class="panel-title"><strong>Process Flow</strong><span>Use these checkpoints to see where records should move next</span></div></div>
      <div class="workflow-grid">
        ${workflow("Purchasing", ["PO issued", "Goods receipt", "AP posted", "Check run"])}
        ${workflow("Sales", ["Order approved", "Inventory issued", "Invoice", "Collection"])}
        ${workflow("Repairs", ["Open WO", "Parts/labor", "Ready to close", "Invoice"])}
        ${workflow("Accounting", ["Post", "Review", "Reconcile", "Close period"])}
      </div>
    </section>
    <section class="panel">
      ...
      <div class="alert-list">
        ${alerts.map((alert) => `<button class="alert-row ${esc(alert.level)}" data-jump="${esc(alert.view)}"><span>${esc(alert.label)}</span><strong>Review</strong></button>`).join("")}
      </div>
    </section>
    <section class="panel">
...
async function getDashboardRows(table, columns = "*", limit = 1000) {
  try {
    ...
    if (error) {
      console.warn(`Dashboard skipped ${table}:`, error.message || error);
      return [];
    }
    return data || [];
  } catch (error) {
...
function workflow(title, steps) {
  return `<div class="workflow-card"><strong>${esc(title)}</strong><div class="workflow-steps">${steps.map((step, index) => `<span>${index + 1}. ${esc(step)}</span>`).join("")}</div></div>`;
}
  ...
  $("content").innerHTML = `
    ...
    <section class="panel">
      ...
      <div class="panel-head">
        ...
        <div class="actions grouped-actions accounting-actions">
          <div class="action-cluster date-cluster">
            <span class="cluster-label">Period</span>
            ...
            <button id="accountingApplyDatesBtn">Apply</button>
          </div>
          <div class="action-cluster">
            <span class="cluster-label">Create</span>
            <button id="newAccountBtn">Account</button>
            <button id="newBankItemBtn">Bank item</button>
            <button class="primary" id="newJournalBtn">Journal</button>
          </div>
          <details class="inline-action-menu">
            <summary>Beginning balances</summary>
            <div class="inline-action-panel">
              ...
              <button id="beginningInventoryTemplateBtn">Inventory template</button>
              <button id="beginningInventoryUploadBtn">Upload inventory</button>
              <button id="beginningApTemplateBtn">AP template</button>
              <button id="beginningApUploadBtn">Upload AP</button>
              <button id="beginningArTemplateBtn">AR template</button>
              <button id="beginningArUploadBtn">Upload AR</button>
            </div>
          </details>
          <div class="action-cluster">
            <span class="cluster-label">Export</span>
...
async function importBeginningApFile() {
  ...
  try {
    ...
    const payables = rows.map(beginningApRowToRecord).filter((row) => row.vendor && row.vendor_invoice_amount > 0);
...
async function importBeginningArFile() {
  ...
  try {
    ...
    const receivables = rows.map(beginningArRowToRecord).filter((row) => row.customer && row.amount > 0);
...
async function readBeginningDetailRows(file, preferredSheet) {
  ...
  if (name.endsWith(".csv")) return parseCsv(await file.text()).map(normalizeAssetTemplateRow);
  if (name.endsWith(".xlsx") || name.endsWith(".xls")) {
    try {
      const XLSX = await import("https://cdn.sheetjs.com/xlsx-0.20.3/package/xlsx.mjs");
      const workbook = XLSX.read(await file.arrayBuffer(), { type: "array" });
      const sheet = workbook.Sheets[preferredSheet] || workbook.Sheets[workbook.SheetNames[0]];
      return XLSX.utils.sheet_to_json(sheet, { defval: "" }).map(normalizeAssetTemplateRow);
    } catch (error) {
...
function beginningApRowToRecord(row) {
  ...
  const vendor = String(get("vendor", "vendor_name", "supplier")).trim();
  ...
  const amount = nullableNumber(get("amount", "invoice_amount", "balance", "open_balance")) || 0;
...
function beginningArRowToRecord(row) {
  const get = (...keys) => keys.map((key) => row[key]).find((value) => String(value ?? "").trim() !== "") ?? "";
  const customer = String(get("customer", "customer_name")).trim();
  ...
  return {
    ...
    amount: nullableNumber(get("amount", "invoice_amount", "balance", "open_balance")) || 0,
...
async function replaceBeginningArDetail(receivables, postingDate) {
  ...
  const old = existing.filter((row) => /beginning ar/i.test(`${row.type || ""} ${row.notes || ""}`));
  if (old.length) {
  ...
  const invoices = await upsertMany("invoices", receivables.map((row, index) => ({
    invoice_no: row.invoice_no || `BBAR-${postingDate}-${index + 1}`,
...
function trialBalanceRowToLedger(row) {
  ...
  const account = String(get("account", "account_name", "name")).trim();
...
async function renderCheckRunView() {
  ...
  const [pos, poLines, receipts, runs, coa, gl] = await Promise.all([
    getAll("purchase_orders"),
    getAll("purchase_order_lines"),
    getAll("goods_receipts"),
    getAll("check_runs"),
...
function checkPrintHtml(row, vendor, invoices, amount, userNotes = "") {
  ...
  const checkNo = row.check_no || row.check_run_no || "";
  ...
  const invoiceRows = invoices.map((inv) => `<tr>
    ...
    <td>${esc(formatShortDate(inv.due_date || ""))}</td>
  ...
  return `<html><head><title>Check ${esc(checkNo)}</title><style>
    ...
    *{box-sizing:border-box}
    ...
    .voucher-table th:nth-child(5),.voucher-table th:nth-child(7),.voucher-table td:nth-child(5),.voucher-table td:nth-child(7){text-align:right}
    ...
    .check-no-repeat{position:absolute;right:.05in;top:-.18in;font-weight:700;font-size:13px}
    ...
    <section class="voucher-top">
      ...
      <table class="voucher-table">
        <thead><tr><th>Invoice #</th><th>Invoice Date</th><th>Due Date</th><th>Reference</th><th>Gross</th><th>Discount</th><th>Net Paid</th></tr></thead>
...
async function renderControlCenterView() {
  $("viewTitle").textContent = "Control Center";
  $("viewSub").textContent = "Audit, approvals, notifications, documents, period close, numbering, imports, cleanup, and backup controls.";
  const [notifications, approvals, audit, docs, periods, sequences, backups, imports, issues, products, vendors, customers, assets] = await Promise.all([
    getAll("system_notifications"),
    getAll("approval_queue"),
    getAll("audit_log"),
    getAll("document_attachments"),
    getAll("accounting_periods"),
    getAll("numbering_sequences"),
    getAll("backup_exports"),
    getAll("import_reviews"),
    getAll("master_data_issues"),
    getAll("products"),
    getAll("vendors"),
    getAll("customers"),
    getAll("assets"),
  ]);
  const openNotifications = notifications.filter((r) => !/resolved|closed|done/i.test(r.status || ""));
  const pendingApprovals = approvals.filter((r) => !/approved|denied|closed|done/i.test(r.status || ""));
  const openIssues = [...issues.filter((r) => !/resolved|closed|done/i.test(r.status || "")), ...findLiveMasterIssues(products, vendors, customers, assets)];
  const lastBackup = backups.sort((a, b) => String(b.created_at || "").localeCompare(String(a.created_at || "")))[0];
  const closeDate = getAccountingCloseDate();
  $("content").innerHTML = `
    <section class="stats compact-stats">
      <div class="stat"><span>Pending Approvals</span><strong>${pendingApprovals.length}</strong><small>Manager/accounting review</small></div>
      <div class="stat"><span>Open Notifications</span><strong>${openNotifications.length}</strong><small>Exceptions and reminders</small></div>
      <div class="stat"><span>Master Issues</span><strong>${openIssues.length}</strong><small>Duplicates or blanks</small></div>
      <div class="stat"><span>Period Close</span><strong>${esc(closeDate || "Open")}</strong><small>Posting lock status</small></div>
    </section>
    <section class="control-grid">
      ${controlCard("Approval Workflow", "Review equipment requests, overrides, AP holds, and work-order close approvals before posting.", pendingApprovals.length, "approvals", "Open approvals")}
      ${controlCard("Notification Center", "Operational reminders for low stock, payables, AR follow-up, repair readiness, and bank review.", openNotifications.length, "notifications", "Open notifications")}
      ${controlCard("Document Attachments", "Track signed POs, customer POs, invoices, photos, and approval documents by module reference.", docs.length, "documents", "Open documents")}
      ${controlCard("Audit Log", "Every supported create, update, and delete now records who changed it and the module reference.", audit.length, "auditlog", "Open audit log")}
      ${controlCard("Period Close", "Lock prior posting dates so AP, AR, GL, checks, and inventory cannot be changed accidentally.", closeDate || "Open", "settings", "Open settings")}
      ${controlCard("Backup Register", `Last backup: ${lastBackup ? esc(lastBackup.created_at || lastBackup.backup_date || "") : "Not recorded"}`, backups.length, "", "Record backup", "backupNowBtn")}
      ${controlCard("Import Review", "Log uploads, rejected rows, duplicates, and corrections for products, vendors, customers, assets, and trial balances.", imports.length, "", "Record import review", "importReviewBtn")}
      ${controlCard("Master Data Cleanup", "Find duplicate SKUs, duplicate references, blank names, missing vendors, and untagged assets before they affect reports.", openIssues.length, "", "Refresh issues", "refreshIssuesBtn")}
    </section>
    <section class="panel">
      <div class="panel-head"><div class="panel-title"><strong>Numbering Setup</strong><span>Reference sequence setup for invoices, work orders, POs, receipts, checks, and requests.</span></div><div class="actions"><button id="seedSequencesBtn">Seed standard numbering</button></div></div>
      <div class="table-wrap">${simpleTable(["Key", "Prefix", "Next Number", "Notes"], sequenceRows(sequences))}</div>
    </section>
    <section class="grid-2">
      <div class="panel"><div class="panel-head"><div class="panel-title"><strong>Recent Audit</strong><span>Latest system changes</span></div></div>${compactList(audit.slice(-8).reverse(), (r) => `${r.event_at || ""} - ${r.user_name || "User"} - ${r.summary || r.event_type || ""}`)}</div>
      <div class="panel"><div class="panel-head"><div class="panel-title"><strong>Cleanup Watchlist</strong><span>Master data items to review</span></div></div>${compactList(openIssues.slice(0, 8), (r) => `${r.module || "Master"} - ${r.issue || r.summary || ""}`)}</div>
    </section>`;
  document.querySelectorAll("[data-control-view]").forEach((btn) => btn.onclick = () => loadView(btn.dataset.controlView));
  $("backupNowBtn")?.addEventListener("click", recordBackupExport);
  $("importReviewBtn")?.addEventListener("click", recordImportReview);
  $("refreshIssuesBtn")?.addEventListener("click", refreshMasterDataIssues);
  $("seedSequencesBtn")?.addEventListener("click", seedStandardNumbering);
}

function controlCard(title, text, count, view, buttonLabel, buttonId = "") {
  const action = view
    ? `<button data-control-view="${esc(view)}">${esc(buttonLabel)}</button>`
    : `<button id="${esc(buttonId)}">${esc(buttonLabel)}</button>`;
  return `<article class="control-card"><div><span>${esc(title)}</span><strong>${esc(count)}</strong><p>${text}</p></div>${action}</article>`;
}

function simpleTable(headers, rows) {
  return `<table class="compact-table"><thead><tr>${headers.map((h) => `<th>${esc(h)}</th>`).join("")}</tr></thead><tbody>${rows.length ? rows.map((row) => `<tr>${row.map((v) => `<td>${esc(v)}</td>`).join("")}</tr>`).join("") : `<tr><td colspan="${headers.length}" class="empty">No setup rows yet.</td></tr>`}</tbody></table>`;
}

function compactList(rows, getText) {
  if (!rows.length) return `<div class="empty">No open items.</div>`;
  return `<div class="compact-list">${rows.map((row) => `<div>${esc(getText(row))}</div>`).join("")}</div>`;
}

function sequenceRows(rows) {
  const defaults = [
    ["purchase_order", "PO-", "1001", "Purchase orders"],
    ["goods_receipt", "GR-", "6001", "Goods receipts"],
    ["work_order", "WO-", "1001", "Work orders"],
    ["work_order_invoice", "WO-", "10001", "Work order invoices"],
    ["parts_invoice", "P", "10001", "Parts invoices"],
    ["misc_invoice", "M", "10001", "Other sales invoices"],
    ["check_run", "CHK-", "1001", "Check runs"],
    ["equipment_request", "ER-", "1001", "Equipment requests"],
  ];
  const byKey = Object.fromEntries(rows.map((r) => [r.sequence_key || r.key || r.name, r]));
  return defaults.map(([key, prefix, next, notes]) => {
    const row = byKey[key] || {};
    return [key, row.prefix || prefix, row.next_number || next, row.notes || notes];
  });
}

function findLiveMasterIssues(products, vendors, customers, assets) {
  const issues = [];
  duplicateWatch(products, "Products", "sku", "name").forEach((issue) => issues.push(issue));
  duplicateWatch(vendors, "Vendors", "reference", "name").forEach((issue) => issues.push(issue));
  duplicateWatch(customers, "Customers", "reference", "name").forEach((issue) => issues.push(issue));
  duplicateWatch(assets, "Fleet & Equipment", "asset_tag", "name").forEach((issue) => issues.push(issue));
  products.filter((p) => !p.name || !p.sku).slice(0, 10).forEach((p) => issues.push({ module: "Products", issue: `Missing SKU or name: ${p.sku || p.name || "blank row"}` }));
  assets.filter((a) => !a.asset_tag || !a.name).slice(0, 10).forEach((a) => issues.push({ module: "Fleet & Equipment", issue: `Missing asset # or description: ${a.asset_tag || a.name || "blank row"}` }));
  return issues;
}

function duplicateWatch(rows, module, key, label) {
  const seen = new Map();
  rows.forEach((row) => {
    const value = String(row[key] || "").trim().toLowerCase();
    if (!value) return;
    seen.set(value, [...(seen.get(value) || []), row]);
  });
  return [...seen.entries()]
    .filter(([, matches]) => matches.length > 1)
    .map(([value, matches]) => ({ module, issue: `Duplicate ${key}: ${value} (${matches.map((r) => r[label] || r[key]).join(", ")})` }));
}

async function recordBackupExport() {
  await supabase.from("backup_exports").insert({
    created_at: new Date().toISOString(),
    backup_type: "Manual",
    created_by: activeUserName(),
    notes: "Backup register entry. Use Export Clone to download the actual file.",
    status: "Recorded",
  });
  await logAudit("Backup", "Control Center", "Manual backup", "Recorded a manual backup checkpoint");
  alert("Backup checkpoint recorded. Use Export Clone if you want the downloadable copy.");
  renderControlCenterView();
}

async function recordImportReview() {
  const module = prompt("Which module was imported? Example: Products, Vendors, Assets, Trial Balance");
  if (!module) return;
  await supabase.from("import_reviews").insert({
    created_at: new Date().toISOString(),
    module,
    file_name: "",
    imported_by: activeUserName(),
    total_rows: 0,
    accepted_rows: 0,
    rejected_rows: 0,
    status: "Review",
    notes: "Manual import review entry.",
  });
  await logAudit("Import Review", "Control Center", module, `Recorded import review for ${module}`);
  renderControlCenterView();
}

async function refreshMasterDataIssues() {
  const [products, vendors, customers, assets] = await Promise.all([getAll("products"), getAll("vendors"), getAll("customers"), getAll("assets")]);
  const rows = findLiveMasterIssues(products, vendors, customers, assets).map((issue) => ({
    created_at: new Date().toISOString(),
    module: issue.module,
    issue: issue.issue,
    severity: "Review",
  ...
  await supabase.from("master_data_issues").delete().neq("id", "00000000-0000-0000-0000-000000000000");
  if (rows.length) await supabase.from("master_data_issues").insert(rows);
  await logAudit("Cleanup", "Control Center", "Master data", `Refreshed ${rows.length} master data issue(s)`);
  renderControlCenterView();
}

async function seedStandardNumbering() {
  const rows = sequenceRows([]).map(([sequence_key, prefix, next_number, notes]) => ({ sequence_key, prefix, next_number: Number(next_number), notes }));
  await supabase.from("numbering_sequences").upsert(rows, { onConflict: "sequence_key" });
  await logAudit("Setup", "Control Center", "Numbering", "Seeded standard numbering sequences");
  renderControlCenterView();
}
...
async function renderProductsView() {
  ...
  $("content").innerHTML = `
    ...
    <section class="panel">
      <div class="panel-head"><div class="panel-title"><strong>Product Master</strong><span>${currentRows.length} items available. Fast view draws ${Math.min(PRODUCT_RENDER_LIMIT, currentRows.length)} rows at a time; search narrows the full list.</span></div><div class="actions"><button id="productColumnsBtn">Columns</button><button id="productTemplateBtn">Download product template</button><button id="productUploadBtn">Upload product list</button><button id="productQtyCostTemplateBtn">Download qty/cost template</button><button id="productQtyCostUploadBtn">Upload qty/cost update</button><button id="productsCsvBtn">Excel</button><button onclick="window.print()">PDF / Print</button></div></div>
...
function purchaseOrderRowHtml(po) {
  ...
  return `<tr>
    ...
    <td><div class="rowactions">${landedAp ? "" : `<button class="rowbtn" type="button" data-po-pdf="${esc(po.po_no)}">PDF</button><button class="rowbtn" type="button" data-po-email="${esc(po.po_no)}">Email</button><button class="rowbtn" type="button" data-po-gr="${esc(po.po_no)}">GR</button><button class="rowbtn" type="button" data-po-landed="${esc(po.po_no)}">Landed AP</button><button class="rowbtn" type="button" data-po-match="${esc(po.po_no)}">=</button>`}${canPay ? `<button class="rowbtn" type="button" data-po-pay="${esc(po.po_no)}">$</button>` : ""}<button class="rowbtn" type="button" data-po-edit="${esc(po.po_no)}">Edit</button><button class="rowbtn danger" type="button" data-po-delete="${esc(po.po_no)}">Delete</button></div></td>
  </tr>`;
}
...
function setupImageDropzones(root = document) {
  root.querySelectorAll('input[type="file"][accept^="image/"]').forEach((input) => {
    ...
    input.addEventListener("change", () => previewDroppedImage(input, field));
  });
}
...
function exportAssetsWithQrCsv() {
  ...
  const csvRows = [heads, ...rows.map((a) => {
    ...
    return columns.map(([key]) => {
      ...
      if (key === "photo") return a.photo_url || "";
...
function sheetToRowsWithDetectedHeader(XLSX, sheet, headerCandidates = []) {
  ...
  const normalizedCandidates = headerCandidates.map((header) => normalizeHeaderKey(header));
  const headerIndex = matrix.findIndex((row) => {
    const headers = row.map((cell) => normalizeHeaderKey(cell));
    return normalizedCandidates.some((candidate) => headers.includes(candidate));
  });
  if (headerIndex < 0) return XLSX.utils.sheet_to_json(sheet, { defval: "" }).map(normalizeAssetTemplateRow);
  const headers = matrix[headerIndex].map((cell) => normalizeHeaderKey(cell));
...
function contactMasterRowToDb(row, prefix, batch, index) {
  ...
  return {
    ...
    name: String(get("name", "vendor_name", "supplier_name", "customer_name", "company_name")).trim(),
...
function rowHtml(row) {
  ...
  return `<tr>${currentCfg.heads.map((h) => `<td>${cell(h, row[h], row)}</td>`).join("")}<td><div class="rowactions">${genericRowActions(row, key)}</div></td></tr>`;
}

function genericRowActions(row, key) {
  if (currentView === "periods") {
    const closed = /closed|locked/i.test(row.status || "");
    return `${closed ? `<button class="rowbtn" data-period-reopen="${esc(key)}">Reopen</button>` : `<button class="rowbtn" data-period-close="${esc(key)}">Close</button>`}<button class="rowbtn" data-edit="${esc(key)}">Edit</button>`;
  }
  if (currentView === "approvals") {
    const closed = /approved|denied|closed|done/i.test(row.status || "");
    return `${closed ? "" : `<button class="rowbtn" data-approval-approve="${esc(key)}">Approve</button><button class="rowbtn danger" data-approval-deny="${esc(key)}">Deny</button>`}<button class="rowbtn" data-edit="${esc(key)}">Edit</button>`;
  }
  if (currentView === "notifications") {
    const closed = /resolved|closed|done/i.test(row.status || "");
    return `${closed ? "" : `<button class="rowbtn" data-notification-resolve="${esc(key)}">Resolve</button>`}<button class="rowbtn" data-edit="${esc(key)}">Edit</button>`;
  }
  if (currentView === "documents") {
    const link = row.file_url ? `<button class="rowbtn" data-document-open="${esc(row.file_url)}">Open</button>` : "";
    return `${link}<button class="rowbtn" data-edit="${esc(key)}">Edit</button><button class="rowbtn danger" data-del="${esc(key)}">Delete</button>`;
  }
  if (currentCfg.readOnly) return "";
  return `<button class="rowbtn" data-edit="${esc(key)}">Edit</button><button class="rowbtn danger" data-del="${esc(key)}">Delete</button>`;
}

function cell(head, value) {
  if (head === "file_url") return value ? `<button class="rowbtn" data-document-open="${esc(value)}">Open</button>` : "";
...
function bindRows() {
  ...
  document.querySelectorAll("[data-approval-approve]").forEach((b) => b.onclick = () => updateApprovalStatus(b.dataset.approvalApprove, "Approved"));
  document.querySelectorAll("[data-approval-deny]").forEach((b) => b.onclick = () => updateApprovalStatus(b.dataset.approvalDeny, "Denied"));
  document.querySelectorAll("[data-notification-resolve]").forEach((b) => b.onclick = () => resolveNotification(b.dataset.notificationResolve));
  document.querySelectorAll("[data-document-open]").forEach((b) => b.onclick = () => window.open(b.dataset.documentOpen, "_blank", "noopener"));
  document.querySelectorAll("[data-period-close]").forEach((b) => b.onclick = () => updatePeriodStatus(b.dataset.periodClose, "Closed"));
  document.querySelectorAll("[data-period-reopen]").forEach((b) => b.onclick = () => updatePeriodStatus(b.dataset.periodReopen, "Open"));
  document.querySelectorAll(".column-filter").forEach((input) => input.oninput = applyColumnFilters);
  enhanceColumnFilters();
}

async function updatePeriodStatus(periodName, status) {
  const row = currentRows.find((r) => String(r.period_name || r.id) === String(periodName));
  if (!row) return alert("Could not find this accounting period. Please refresh and try again.");
  if (status === "Closed") {
    const through = row.closed_through_date || row.end_date;
    if (!through) return alert("Add an End Date or Closed Through date first, then close the period.");
    if (!confirm(`Close accounting period ${row.period_name || ""} through ${through}? Transactions on or before this date will be locked.`)) return;
    const update = {
      status: "Closed",
      closed_through_date: through,
      closed_by: activeUserName(),
      closed_at: new Date().toISOString(),
    };
    const { error } = await supabase.from("accounting_periods").update(update).eq("period_name", periodName);
    if (error) return alert(error.message || error);
    accountingCloseDateCache = through;
    localStorage.setItem("lms.accountingCloseDate", through);
    await logAudit("Close Period", "Period Close", periodName, `Closed period through ${through}`, row, update);
  } else {
    if (!confirm(`Reopen accounting period ${row.period_name || ""}? This allows prior-period corrections again.`)) return;
    const update = {
      status: "Open",
      reopened_by: activeUserName(),
      reopened_at: new Date().toISOString(),
    };
    const { error } = await supabase.from("accounting_periods").update(update).eq("period_name", periodName);
    if (error) return alert(error.message || error);
    await loadAccountingCloseDate();
    await logAudit("Reopen Period", "Period Close", periodName, "Reopened accounting period", row, update);
  }
  loadView("periods");
}

async function updateApprovalStatus(id, status) {
  const row = currentRows.find((r) => String(r.id || r[currentCfg.key]) === String(id));
  const notes = status === "Denied" ? prompt("Reason for denial?") : prompt("Approval notes, optional") || "";
  if (status === "Denied" && notes === null) return;
  const update = {
    status,
    notes: [row?.notes, notes].filter(Boolean).join(row?.notes && notes ? "\n" : ""),
    decided_by: activeUserName(),
    decided_at: new Date().toISOString(),
  };
  if (status === "Approved") {
    update.approved_by = activeUserName();
    update.approved_at = update.decided_at;
  }
  const { error } = await supabase.from("approval_queue").update(update).eq("id", id);
  if (error) return alert(error.message || error);
  await logAudit(status, "Approvals", row?.record_ref || id, `${status} approval ${row?.record_ref || ""}`, row, update);
  await supabase.from("system_notifications").insert({
    created_at: new Date().toISOString(),
    severity: status === "Approved" ? "Info" : "Warning",
    module: row?.module || "Approvals",
    title: `Approval ${status}`,
    message: `${row?.approval_type || "Request"} ${status.toLowerCase()} by ${activeUserName()}`,
...
async function resolveNotification(id) {
  const row = currentRows.find((r) => String(r.id || r[currentCfg.key]) === String(id));
  const update = { status: "Resolved", resolved_at: new Date().toISOString(), assigned_to: row?.assigned_to || activeUserName() };
  const { error } = await supabase.from("system_notifications").update(update).eq("id", id);
  if (error) return alert(error.message || error);
  await logAudit("Resolve", "Notifications", row?.record_ref || id, `Resolved notification ${row?.title || ""}`, row, update);
  loadView("notifications");
}

function enhanceColumnFilters() {
  document.querySelectorAll(".table-wrap table, .asset-table-wrap table").forEach((table) => ensureTableViewTools(table));
  document.querySelectorAll(".column-filter").forEach((input) => {
    if (input.dataset.filterEnhanced === "1") return;
    const col = esc(input.dataset.col || "");
    input.dataset.filterEnhanced = "1";
    input.insertAdjacentHTML("afterend", `
      <select class="column-filter-mode" data-col="${col}" title="Choose how this column filter matches">
      ...
      <select class="column-sort" data-col="${col}" title="Sort this column">
        <option value="">No sort</option>
        <option value="asc">A to Z / Low to high</option>
        <option value="desc">Z to A / High to low</option>
      </select>
    `);
    const mode = input.nextElementSibling;
    const sort = mode?.nextElementSibling;
    if (mode) mode.onchange = applyColumnFilters;
    if (sort) sort.onchange = applyColumnFilters;
  });
  enhanceRowActions();
}
...
function applyColumnFilters(event) {
  ...
  const filters = [...table.querySelectorAll(".column-filter")].map((input) => {
    ...
    const mode = table.querySelector(`.column-filter-mode[data-col="${CSS.escape(col)}"]`)?.value || "contains";
    return { col: Number(col), value: input.value.toLowerCase().trim(), mode };
  });
  ...
  [...tbody.querySelectorAll("tr")].forEach((tr) => {
    ...
    tr.style.display = filters.every((filter) => {
      if (Number.isNaN(filter.col)) return true;
      const cellText = (cells[filter.col]?.textContent || "").replace(/\s+/g, " ").trim().toLowerCase();
      if (filter.mode === "blank") return !cellText;
      if (filter.mode === "notblank") return !!cellText;
      if (!filter.value) return true;
      if (filter.mode === "exact") return cellText === filter.value;
      if (filter.mode === "starts") return cellText.startsWith(filter.value);
...
function tableViewKey(table) {
  if (table.dataset.savedViewKey) return table.dataset.savedViewKey;
  const tables = [...document.querySelectorAll("#content .table-wrap table, #content .asset-table-wrap table")];
  const labels = [...table.querySelectorAll("thead tr:first-child th")].map((th) => th.textContent.trim()).join("|").slice(0, 120);
  table.dataset.savedViewKey = `${currentView || "view"}:${tables.indexOf(table)}:${labels}`;
  return table.dataset.savedViewKey;
}

function tableViewStore() {
  try { return JSON.parse(localStorage.getItem(TABLE_VIEW_STORAGE_KEY) || "{}"); } catch { return {}; }
}

function saveTableViewStore(store) {
  localStorage.setItem(TABLE_VIEW_STORAGE_KEY, JSON.stringify(store || {}));
}

function ensureTableViewTools(table) {
  const wrap = table.closest(".table-wrap, .asset-table-wrap");
  if (!wrap || wrap.previousElementSibling?.classList?.contains("saved-view-tools")) return;
  const key = tableViewKey(table);
  const store = tableViewStore();
  const views = Object.keys(store[key] || {});
  const html = `
    <div class="saved-view-tools" data-table-view-key="${esc(key)}">
      <div class="saved-view-left">
        <span class="saved-view-label">Saved views</span>
        <select class="saved-view-select"><option value="">Select view</option>${views.map((name) => `<option value="${esc(name)}">${esc(name)}</option>`).join("")}</select>
      </div>
      <div class="saved-view-actions">
        <button type="button" class="saved-view-apply">Apply</button>
        <button type="button" class="saved-view-save">Save current</button>
        <button type="button" class="saved-view-clear">Clear filters</button>
      </div>
    </div>`;
  wrap.insertAdjacentHTML("beforebegin", html);
  const tools = wrap.previousElementSibling;
  tools.querySelector(".saved-view-apply").onclick = () => applySavedTableView(table, tools.querySelector(".saved-view-select").value);
  tools.querySelector(".saved-view-save").onclick = () => saveCurrentTableView(table, tools);
  tools.querySelector(".saved-view-clear").onclick = () => clearTableFilters(table);
}

function serializeTableView(table) {
  return {
    filters: [...table.querySelectorAll(".column-filter")].map((input) => ({ col: input.dataset.col || "", value: input.value || "" })),
    modes: [...table.querySelectorAll(".column-filter-mode")].map((select) => ({ col: select.dataset.col || "", value: select.value || "contains" })),
    sort: [...table.querySelectorAll(".column-sort")].map((select) => ({ col: select.dataset.col || "", value: select.value || "" })),
  };
}

function applySavedTableView(table, name) {
  const key = tableViewKey(table);
  const saved = tableViewStore()[key]?.[name];
  if (!saved) return;
  saved.filters?.forEach((item) => {
    const input = table.querySelector(`.column-filter[data-col="${CSS.escape(item.col)}"]`);
    if (input) input.value = item.value || "";
  });
  saved.modes?.forEach((item) => {
    const select = table.querySelector(`.column-filter-mode[data-col="${CSS.escape(item.col)}"]`);
    if (select) select.value = item.value || "contains";
  });
  saved.sort?.forEach((item) => {
    const select = table.querySelector(`.column-sort[data-col="${CSS.escape(item.col)}"]`);
    if (select) select.value = item.value || "";
  });
  applyColumnFilters({ target: table });
}

function saveCurrentTableView(table, tools) {
  const name = prompt("Name this saved view:");
  if (!name?.trim()) return;
  const key = tableViewKey(table);
  const store = tableViewStore();
  store[key] = store[key] || {};
  store[key][name.trim()] = serializeTableView(table);
  saveTableViewStore(store);
  const select = tools.querySelector(".saved-view-select");
  if (select && ![...select.options].some((option) => option.value === name.trim())) {
    select.insertAdjacentHTML("beforeend", `<option value="${esc(name.trim())}">${esc(name.trim())}</option>`);
  }
...
function clearTableFilters(table) {
  ...
  table.querySelectorAll(".column-filter-mode").forEach((select) => select.value = "contains");
  table.querySelectorAll(".column-sort").forEach((select) => select.value = "");
  applyColumnFilters({ target: table });
}

function enhanceRowActions() {
  document.querySelectorAll(".rowactions").forEach((actions) => {
    if (actions.dataset.menuEnhanced === "1") return;
    const buttons = [...actions.children].filter((child) => child.tagName === "BUTTON");
    if (buttons.length < 3) {
      actions.dataset.menuEnhanced = "1";
      return;
    }
    actions.dataset.menuEnhanced = "1";
    const menu = document.createElement("details");
    menu.className = "row-action-menu";
    menu.innerHTML = `<summary>Actions</summary><div class="row-action-panel"></div>`;
    const panel = menu.querySelector(".row-action-panel");
    const detailButton = document.createElement("button");
    detailButton.type = "button";
    detailButton.className = "rowbtn";
    detailButton.textContent = "Details";
    detailButton.onclick = () => openRowDetailPanel(actions.closest("tr"));
...
function openRowDetailPanel(row) {
  if (!row) return;
  const table = row.closest("table");
  const headers = [...(table?.querySelectorAll("thead tr:first-child th") || [])].map((th) => th.innerText.trim());
  const cells = [...row.children].map((cell) => cell.innerText.replace(/\s+/g, " ").trim());
  const pairs = cells.map((value, index) => ({ label: headers[index] || `Column ${index + 1}`, value })).filter((item) => item.value);
  ...
  $("modalBody").innerHTML = `
    <div class="detail-panel">
      ...
      `).join("") || `<p class="muted">No visible row details.</p>`}
    </div>`;
  $("modalSave").style.display = "none";
  $("modalCancel").textContent = "Close";
  $("modal").style.display = "flex";
...
async function openModal(row = null) {
  ...
  bindDocumentAttachmentField(row);
}
...
function formField(h, value) {
  ...
  if (currentView === "documents" && h === "file_url") return documentAttachmentField(value);
...
function documentAttachmentField(value = "") {
  return `
    <div class="field wide attachment-field">
      <label>Attachment</label>
      <input type="hidden" data-field="file_url" value="${esc(value || "")}">
      <div class="file-dropzone document-dropzone">
        <div class="attachment-row">
          <input type="file" data-document-file>
          <button type="button" class="rowbtn" data-document-clear>Clear attachment</button>
        </div>
        <input class="attachment-link" data-document-link value="${esc(value || "")}" placeholder="Choose a file, or paste a document/photo link">
        <div class="file-drop-hint">Drag a file here or choose one. Images are resized before saving; PDF and document files are stored as a compact attachment link when small enough.</div>
        <div class="attachment-preview" data-document-preview>${value ? `<button type="button" class="rowbtn" data-document-open="${esc(value)}">Open current attachment</button>` : "No attachment selected."}</div>
      </div>
    </div>`;
}

function bindDocumentAttachmentField(row = null) {
  if (currentView !== "documents") return;
  const field = document.querySelector(".document-dropzone");
  const fileInput = document.querySelector("[data-document-file]");
  const urlInput = document.querySelector('[data-field="file_url"]');
  const linkInput = document.querySelector("[data-document-link]");
  const preview = document.querySelector("[data-document-preview]");
  const clearBtn = document.querySelector("[data-document-clear]");
  if (!field || !urlInput || !linkInput) return;
  const syncLink = () => {
    urlInput.value = linkInput.value.trim();
    if (preview) preview.innerHTML = urlInput.value ? `<button type="button" class="rowbtn" data-document-open="${esc(urlInput.value)}">Open attachment</button>` : "No attachment selected.";
    preview?.querySelector?.("[data-document-open]")?.addEventListener("click", (event) => window.open(event.currentTarget.dataset.documentOpen, "_blank", "noopener"));
  };
  linkInput.oninput = syncLink;
  ...
  field.addEventListener("drop", async (event) => {
    ...
    if (file) await attachDocumentFile(file, { row, urlInput, linkInput, preview });
  });
  if (fileInput) {
...
async function attachDocumentFile(file, refs) {
  try {
    const prepared = file.type.startsWith("image/") ? await resizeImageForUpload(file, { maxSide: 1200, quality: 0.68 }) : file;
    const maxBytes = prepared.type.startsWith("image/") ? 900000 : 1500000;
    if (prepared.size > maxBytes) {
      alert("This file is still too large for direct attachment. Please use a smaller file or paste a shared file link instead.");
      return;
    }
    const dataUrl = await fileToDataUrl(prepared);
    refs.urlInput.value = dataUrl;
    refs.linkInput.value = dataUrl;
    setDocumentFieldValue("file_name", prepared.name || file.name);
    setDocumentFieldValue("file_type", prepared.type || file.type || "file");
    setDocumentFieldValue("file_size", String(prepared.size || file.size || ""));
    setDocumentFieldValue("uploaded_by", activeUserName());
    if (refs.preview) {
      refs.preview.innerHTML = prepared.type.startsWith("image/")
        ? `<img class="thumb large" src="${esc(dataUrl)}" alt="Attachment preview"><span>${esc(prepared.name || file.name)}</span>`
        : `<button type="button" class="rowbtn" data-document-open="${esc(dataUrl)}">Open ${esc(prepared.name || file.name)}</button>`;
      refs.preview.querySelector?.("[data-document-open]")?.addEventListener("click", (event) => window.open(event.currentTarget.dataset.documentOpen, "_blank", "noopener"));
    }
  } catch (error) {
    alert(error.message || error);
  }
...
function setDocumentFieldValue(field, value) {
...
function activeUserName() {
  return profile?.username || profile?.full_name || profile?.email || session?.user?.email || "System";
}

async function logAudit(eventType, module, recordRef, summary, oldValue = null, newValue = null, source = "app") {
  if (!session) return;
  try {
    const { error } = await supabase.from("audit_log").insert({
      event_at: new Date().toISOString(),
      event_type: eventType,
      module: module || currentCfg?.title || currentView || "System",
    ...
    if (error && !/audit_log|schema cache|relation/i.test(error.message || "")) console.warn(error);
  } catch (error) {
    console.warn(error);
  }
}
...
async function saveModal() {
  if (currentView === "documents") {
    const docLink = document.querySelector("[data-document-link]");
    const docUrl = document.querySelector('[data-field="file_url"]');
    if (docLink && docUrl && docLink.value.trim()) docUrl.value = docLink.value.trim();
    const createdAt = document.querySelector('[data-field="created_at"]');
    if (createdAt && !createdAt.value) createdAt.value = new Date().toISOString();
    const uploadedBy = document.querySelector('[data-field="uploaded_by"]');
    if (uploadedBy && !uploadedBy.value) uploadedBy.value = activeUserName();
  }
  const record = {};
  document.querySelectorAll("[data-field]").forEach((el) => record[el.dataset.field] = el.value);
  if (currentView === "numbering") record.updated_at = new Date().toISOString();
  if (currentView === "periods") {
    if (!record.period_name && record.end_date) record.period_name = String(record.end_date).slice(0, 7);
    if (!record.closed_through_date && /closed|locked/i.test(record.status || "") && record.end_date) record.closed_through_date = record.end_date;
  }
  try {
  ...
  const auditRef = editing
    ? editing[currentCfg.key] || editing.id || record[currentCfg.key] || record.reference || record.name
    : record[currentCfg.key] || record.id || record.reference || record.name || currentCfg.title;
  const auditEvent = editing ? "Update" : "Create";
  ...
  await logAudit(auditEvent, currentCfg.title || currentView, auditRef, `${auditEvent} ${currentCfg.title || currentView}`, editing, record);
  closeModal();
...
async function deleteRow(key) {
  if (!confirm("Delete this record?")) return;
  const oldRow = currentRows.find((r) => String(r[currentCfg.key || "id"] || r.id) === String(key)) || null;
  const { error } = await supabase.from(currentCfg.table).delete().eq(currentCfg.key || "id", key);
  if (error) alert(error.message);
  else await logAudit("Delete", currentCfg.title || currentView, key, `Delete ${currentCfg.title || currentView}`, oldRow, null);
  loadView(currentView);
}
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .modalback { position:fixed; inset:0; background:rgba(16,24,32,.45); display:none; align-items:center; justify-content:center; padding:18px; }
    .modalback.show { display:flex; }
    .modalbox { background:#fff; border-radius:8px; width:min(920px, 96vw); max-height:92vh; overflow:hidden; display:flex; flex-direction:column; }
...
async function deleteWhereIn(table, column, values) {
  ...
  if (!clean.length) return;
  const { error } = await supabase.from(table).delete().in(column, clean);
  if (error) throw new Error(`${table}: ${error.message}`);
}

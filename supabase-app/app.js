...
function openRowHistorySnapshot(button) {
  ...
  const reference = values.find((item) => item.value)?.value || currentView || "Record";
  $("modalTitle").textContent = `Record History: ${reference}`;
  $("modalBody").innerHTML = `
    <div class="audit-note">
      <strong>Visible record snapshot</strong>
      <span>This shows the current record details from the screen. A permanent audit log can be added next with database history for create, edit, approve, reverse, void, and delete actions.</span>
    </div>

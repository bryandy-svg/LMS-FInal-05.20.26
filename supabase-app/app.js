...
const tableMap = {
  ...
  assets: { table: "assets", key: "asset_tag", title: "Fleet & Equipment", sub: "Track vehicles, heavy equipment, readings, ownership, locations, photos, QR codes, requests, and parent/child attachments.", heads: ["photo_url", "qr_update_url", "old_qr_code", "asset_tag", "name", "type", "general_type", "parent_asset_tag", "relationship_type", "compatible_with", "make", "model", "color", "vin_serial", "plate", "location", "last_update_date", "scanned_date", "odometer", "engine_hours", "assigned_operator", "requested_by", "approved_by", "status"], labels: ["Photo", "New QR", "Old QR Code", "Asset #", "Description", "Type", "General Type", "Parent Asset", "Relationship", "Compatible With", "Make", "Model", "Color", "VIN / Serial", "Plate", "Location", "Last Update Date", "Scanned Date", "Odometer", "Hours", "Operator", "Requested By", "Approved By", "Status"] },
...
const assetColumnDefs = [
  ...
  ["last_update_date", "Last Update Date"], ["scanned_date", "Scanned Date"], ["reading", "Reading"], ["assigned_operator", "Operator"], ["requested_by", "Requested By"], ["approved_by", "Approved By"], ["open_wo_issue", "WO / Issue"], ["status", "Status"],
];
const ASSET_COLUMN_STORAGE_KEY = "lms.assetColumns.v4";
...
async function handleAssetHash() {
  ...
  if (!session) return renderPublicAssetScan(assetTag);
  await renderAssetsView();
  const asset = currentRows.find((row) => row.asset_tag === assetTag);

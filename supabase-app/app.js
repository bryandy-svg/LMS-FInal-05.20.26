...
function checkPrintHtml(row, vendor, invoices, amount, userNotes = "") {
  ...
  const payeeAddress = [row.vendor, vendor?.address, vendor?.phone ? `Phone: ${vendor.phone}` : ""].filter(Boolean).join("<br>");
  ...
  return `<html><head><title>Check ${esc(checkNo)}</title><style>
    ...
    *{box-sizing:border-box}
    ...
    .page{width:8.5in;height:11in;padding:.18in .35in .25in .35in;position:relative}
    .voucher-top{height:4.95in;position:relative}
    .voucher-meta{display:grid;grid-template-columns:1fr auto auto auto;gap:.18in;align-items:start;font-size:12px;margin-bottom:.22in}
    .voucher-table{width:100%;border-collapse:collapse;table-layout:fixed}
    .voucher-table th{font-size:11px;text-align:left;border-bottom:1px solid #222;padding:4px 5px;text-transform:uppercase}
    .voucher-table td{padding:5px;border-bottom:1px solid #d8d8d8;vertical-align:top}
    .voucher-table td:nth-child(3),.voucher-table td:nth-child(5){text-align:right}
    .voucher-total{display:grid;grid-template-columns:1fr 1.1in 1.1in .95in;margin-top:.15in;font-weight:700;font-size:13px}
    .voucher-total div{text-align:right;border-top:1px solid #222;padding-top:5px}
    .notes{margin-top:.15in;font-size:11px;color:#333}
    .check-no-repeat{position:absolute;right:.38in;top:5.35in;font-weight:700;font-size:14px}
    .check-body{position:absolute;left:.35in;right:.35in;top:5.92in;height:3.2in}
    .check-date{position:absolute;right:1.65in;top:0;font-size:14px}
    .check-amount{position:absolute;right:.15in;top:0;font-size:14px;font-weight:700}
    .words{position:absolute;left:1.35in;right:.25in;top:.42in;border-bottom:1px solid #111;padding-bottom:4px;font-size:13px;text-transform:uppercase}
    .payee{position:absolute;left:1.35in;top:1.05in;font-size:13px;line-height:1.35}
    .memo{position:absolute;left:.15in;bottom:.2in;font-size:11px;color:#333}
    .signature{position:absolute;right:.2in;bottom:.25in;width:2.4in;border-top:1px solid #111;text-align:center;padding-top:4px;font-size:10px}
    @media print{body{background:#fff}.page{box-shadow:none}}
  </style></head><body><div class="page">
    <section class="voucher-top">
      <div class="voucher-meta">
        <div>${esc(dateText)}</div>
        <div>${esc(vendorCode)}</div>
        <div>01</div>
        <div>${esc(checkNo)}</div>
      </div>
      <table class="voucher-table">
        <thead><tr><th>Invoice #</th><th>Inv Date</th><th>Amount</th><th>Discount</th><th>Paid</th><th>Voucher #</th></tr></thead>
        <tbody>${invoiceRows}</tbody>
      </table>
      <div class="voucher-total"><span></span><span>Final Totals</span><div>${money(amount)}</div><div>${money(amount)}</div></div>
    ...
    <section class="check-body">
      ...
      <div class="payee">${payeeAddress}</div>
      <div class="memo">Invoices: ${esc(invoices.map((inv) => inv.invoice_no || inv.reference).filter(Boolean).join(", "))}</div>
...
function landedCostReadyForReceipt(po) {
  if (!po?.landed_cost_enabled || poLandedAddOn(po) <= 0) return true;
  return landedCostApTotalForSource(po.po_no) + 0.005 >= poLandedAddOn(po);
}
...
async function openGoodsReceiptModal(po) {
  if (!po) return;
  if (!landedCostReadyForReceipt(po)) {
...
async function openEquipmentRequestModal(row = null) {
  ...
  $("addEquipmentRequestItemBtn").onclick = addEquipmentRequestItemRow;
...
function equipmentRequestItemRowHtml(item = {}, index = 0) {
  ...
  return `<tr>
    ...
    <td><input class="suggest-input" list="assetLocationOptions" data-equipment-request-item="location" data-line-index="${index}" value="${esc(item.location || "")}" placeholder="Location" autocomplete="off">${assetLocationOptionsDatalist()}</td>
...
function equipmentRequestLocationField(value = "") {
  return `<div class="field"><label>Location</label><input class="suggest-input" list="assetLocationOptions" data-equipment-request-field="location" value="${esc(value || "")}" placeholder="Type to search location" autocomplete="off"><button class="rowbtn full" type="button" data-product-quick="asset_locations">New location</button></div>`;
}
...
function parseEquipmentRequestItemRows() {
  ...
  [...document.querySelectorAll("#equipmentRequestItemBody tr")].forEach((tr) => {
    ...
    const location = tr.querySelector('[data-equipment-request-item="location"]')?.value?.trim() || "";

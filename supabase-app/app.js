...
const tableMap = {
  ...
  truckingrequests: { table: "trucking_requests", key: "request_no", title: "Move Requests", sub: "Request trucking moves with delivery timing, service, rate option, origin, destination, and equipment details before dispatch scheduling.", heads: ["request_no", "request_date", "customer", "project_jobsite", "contact_no", "needed_date", "delivery_period", "service_needed", "service_option", "origin", "destination", "requested_by", "requestor_email", "notes"], labels: ["Request #", "Date", "Customer", "Project / Jobsite", "Contact #", "Requested Delivery", "AM / PM", "Service Needed", "Service Option", "Origin", "Destination", "Requested By", "Email", "Move Description"] },
  trucking: { table: "trucking_moves", key: "ticket_no", title: "Trucking Scheduler", sub: "Dispatcher schedules, driver mobile moves, tickets, and billing support without accounting entries.", heads: ["ticket_no", "move_date", "customer", "project", "jobsite", "driver_name", "service", "equipment_label", "origin", "destination", "container_no", "start_time", "end_time", "amount", "status"], labels: ["Ticket #", "Date", "Customer", "Project", "Jobsite", "Driver", "Service", "Equipment", "Origin", "Destination", "Container #", "Start", "End", "Amount", "Status"] },
  truckingrates: { table: "trucking_rates", key: "service", title: "Trucking Rate Sheet", sub: "Organized service groups, selectable rate options, tipping fees, and hourly/flat/daily rules.", heads: ["parent_service", "service", "rate", "rate_type", "additional", "category", "status"], labels: ["Parent Service", "Rate Option / Service", "Rate", "Type", "Additional", "Category", "Status"] },
...
function truckingServiceSelectOptions(selected = "") {
  ...
  const topLevelRates = (productMeta.truckingRates || []).filter((rate) => !rate.parent_service && String(rate.category || "Service") === "Service");
  const values = [...new Set([...standard, ...topLevelRates.map((rate) => rate.service), ...(productMeta.truckingRates || []).map((rate) => rate.parent_service)]
  ...
  if (isTruckingRequestorUser()) {
    return `<div class="truck-mobile-list">${filtered.map((row) => `
      <article class="truck-mobile-card">
        ...
        <small>${esc(row.request_date || "")}${row.needed_date ? ` | Delivery ${esc(row.needed_date)} ${esc(row.delivery_period || "")}` : ""}${row.notes ? ` | ${esc(row.notes)}` : ""}</small>
      </article>`).join("") || `<div class="empty">No move requests yet.</div>`}</div>`;
  }
  return truckingSimpleTable(filtered, ["request_no", "request_date", "customer", "project_jobsite", "contact_no", "needed_date", "delivery_period", "service_needed", "service_option", "origin", "destination", "requested_by", "line_count", "notes"], {
    labels: ["Request #", "Date", "Customer", "Project / Jobsite", "Contact #", "Requested Delivery", "AM / PM", "Service Needed", "Service Option", "Origin", "Destination", "Requested By", "Lines", "Move Description"],
...
async function openTruckingRequestForm(requestNo = "") {
  ...
  modalBody.innerHTML = `
    ...
    <div class="form-grid">
      ...
      <label id="truckServiceOptionField" ${initialRateOptions.length ? "" : "hidden"}>Service option<select name="service_option">${truckingServiceOptionHtml(selectedService, selectedServiceOption)}</select><small>Options and prices come from the Rate Sheet.</small></label>
    ...
    <section class="move-request-equipment">
  ...
  serviceChoice.onchange = () => {
    ...
    const optionField = $("truckServiceOptionField");
    const optionSelect = modalBody.querySelector('[name="service_option"]');
    const options = serviceChoice.value === "__new__" ? [] : truckingRateOptionsForService(serviceChoice.value);
    optionField.hidden = !options.length;
    optionSelect.innerHTML = truckingServiceOptionHtml(serviceChoice.value, "");
  };
  $("modal").style.display = "flex";
...
async function saveTruckingRequest(previousNo = "") {
  ...
  if (requiredRateOptions.length && !values.service_option) return alert(`Select a ${values.service_needed} rate option.`);
  ...
  const lines = [...modalBody.querySelectorAll(".truck-request-line")].map((row, index) => {
    ...
    return {
      ...
      notes: row.querySelector("[name=line_notes]")?.value.trim() || "",
      status: "Requested",
    };
  ...
  if (!lines.length) return alert("Add at least one equipment line.");
...
function truckingMoveTable(rows) {
  ...
  return truckingSimpleTable(filtered, ["ticket_no", "move_date", "customer", "project", "jobsite", "driver_name", "service", "equipment_label", "origin", "destination", "container_no", "start_time", "end_time", "amount", "status"], {
    labels: ["Ticket #", "Date", "Customer", "Project", "Jobsite", "Driver", "Service", "Equipment", "Origin", "Destination", "Container #", "Start", "End", "Amount", "Status"],
...
async function saveTruckingMove() {
  ...
  if (rate) {
    ...
    if (!Number(payload.amount || 0)) payload.amount = Number(rate.rate || 0);
  }
...
async function printTruckingTicket(ticketNo) {
  ...
  const html = `<!doctype html><html><head><title>Ticket ${esc(row.ticket_no)}</title><style>
    ...
    <div class="ticket">
      <div class="head"><div><h2>CONTAINER DELIVERY RECEIPT</h2><h2>TICKET NO: <span class="red">${esc(row.ticket_no)}</span></h2></div><div class="small">Domestic [ ] One-Way [ ] Export [ ]<br>Foreign [ ] Round Trip [ ] Respot [ ]</div><div><div class="logo">LMSGU</div><b>202 Hilton Rd. Tasi Towers B01</b><br>Tumon Bay, Guam 96913<br>Tel: (671) 647-2617<br>Email: trucking@lmsfm.com</div></div>
      <div class="boxgrid">
...
async function renderTruckingRatesView() {
  ...
  $("content").innerHTML = `
    <div class="toolbar"><input class="searchbox" id="truckRateSearch" placeholder="Search service, type, additional, category"></div>
    <section class="panel">
      <div class="panel-head">
...
async function openTruckingRateForm(service = "") {
  ...
  modalBody.innerHTML = `
    <div class="form-grid">
      ...
      <label>Rate option / service<input name="service" value="${esc(row?.service || "")}" placeholder="Example: 0-1,000 gallons"></label>

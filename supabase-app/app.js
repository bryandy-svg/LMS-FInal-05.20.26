...
function accountsPayableRows(data) {
  return data.pos.map((po) => {
    ...
    const apPosted = isBeginningAp || data.gl.some((row) => row.reference === po.po_no && row.source === "Purchase Order");
    return { id: po.id, vendor: po.vendor, po_no: po.po_no, po_date: po.po_date, invoice_no: ap.invoice_no, invoice_date: ap.invoice_date, due_date: ap.due_date, po_total: poTotalAmount, received, invoice_amount: ap.invoice_amount, match: ap.match, payment: ap.payment, status: isBeginningAp ? "Beginning AP" : apPosted ? "AP Posted" : ap.status, ap_posted: apPosted, beginning_ap: isBeginningAp };
...
function poApSummary(po) {
  ...
  const isBeginningAp = /beginning ap/i.test(`${po.status || ""} ${po.notes || ""}`);
  const match = isBeginningAp ? "Matched" : po.match_status && !/pending|awaiting/i.test(po.match_status) ? po.match_status : !received ? "Awaiting Goods" : !invoiceAmount ? "Pending" : Math.abs(invoiceAmount - received) <= 0.005 ? "Matched" : "Mismatch";
...
async function createCheckRunFromPo(data, poNo) {
  ...
  const isBeginningAp = /beginning ap/i.test(`${po.status || ""} ${po.notes || ""}`);
  if (!isBeginningAp && poReceivedTotal(po) <= 0) {
  ...
  if (!isBeginningAp && !await hasPostedApForPurchaseOrder(po.po_no)) {

...
function mechanicVisibleWorkOrders(mechanic) {
  ...
  const rows = currentRows.filter((wo) => {
    if (wo.invoice_no || /invoiced|cancel/i.test(wo.status || "")) return false;
  ...
  if (active && !rows.some((wo) => wo.id === active.id)) rows.unshift(active);
  return rows.length ? rows : currentRows.filter((wo) => !wo.invoice_no && !/closed|complete|invoiced|cancel/i.test(wo.status || ""));
}
    ...
    <section class="mechanic-portal">
      <div class="mechanic-hero">
        <div>
          ...
          <small>${active ? `Clocked in on ${esc(active._wo?.wo_no || "")} since ${formatDateTime(active.clock_in)}` : "Not currently clocked in"}</small>
        </div>
...
function mechanicWorkOrderCard(wo, mechanic, active) {
  ...
  const partsWaiting = (wo._parts || []).filter((p) => /requested|reserved|short|waiting/i.test(effectivePartStatus(p) || p.status || ""));
  return `<article class="mechanic-card ${isActiveHere ? "active" : ""}">
    ...
    <div class="mechanic-card-body">
      ...
      ${partsWaiting.length ? `<div class="mechanic-parts"><strong>Parts</strong>${partsWaiting.map((p) => `<span>${esc(partDisplayName(p))} - ${esc(effectivePartStatus(p))}</span>`).join("")}</div>` : ""}

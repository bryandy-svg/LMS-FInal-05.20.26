...
async function openTruckingRequestForm(requestNo = "") {
  await loadTruckingLookups();
  const existing = requestNo ? (await getAll("trucking_requests")).find((row) => row.request_no === requestNo) : null;
  ...
  const number = existing?.request_no || truckingNextRequestNo(productMeta.truckingRequests || []);
  ...
  modalBody.innerHTML = `
    <div class="form-grid">
      <label>Request #<input name="request_no" value="${esc(number)}" readonly></label>
      <label>Date<input type="date" name="request_date" value="${esc(existing?.request_date || truckingToday())}" readonly></label>
      <label>Customer<input class="suggest-input" name="customer" list="truckCustomerOptions" value="${esc(existing?.customer || "")}" placeholder="Select customer from Customer Master" autocomplete="off" required></label>
      ...
      <label>Service needed<input class="suggest-input" name="service_needed" list="truckServiceOptions" value="${esc(existing?.service_needed || existingLines[0]?.service || "")}" placeholder="${isAdminUser() ? "Select or enter a new service" : "Select a service"}" autocomplete="off" required></label>
  ...
  customerInput.onchange = customerInput.oninput = () => {
    ...
    if (selected && !modalBody.querySelector('[name="contact_no"]').value.trim()) {

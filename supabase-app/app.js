...
const modules = [
  ...
  { group: "Fleet & Repairs", items: [["assets", "Fleet & Equipment"], ["equipmentrequests", "Equipment Requests"], ["equipmentrepairqueue", "Equipment Subject to Repair"], ["equipmenthistory", "Equipment History"], ["repairs", "Repairs"], ["supplies", "Supplies Issuance"], ["partsrequests", "Parts Requests"], ["mechanics", "Mechanics"]] },
...
const tableMap = {
  ...
  assets: { table: "assets", key: "asset_tag", title: "Fleet & Equipment", sub: "Track vehicles, heavy equipment, readings, ownership, locations, photos, QR codes, requests, and parent/child attachments.", heads: ["photo_url", "qr_update_url", "old_qr_code", "asset_tag", "name", "type", "general_type", "parent_asset_tag", "relationship_type", "compatible_with", "make", "model", "color", "vin_serial", "plate", "location", "gps_location", "last_update_date", "scanned_date", "odometer", "engine_hours", "assigned_operator", "requested_by", "approved_by", "repair_po_no", "status"], labels: ["Photo", "New QR", "Old QR Code", "Asset #", "Description", "Type", "General Type", "Parent Asset", "Relationship", "Compatible With", "Make", "Model", "Color", "VIN / Serial", "Plate", "Location", "GPS", "Last Update Date", "Scanned Date", "Odometer", "Hours", "Operator", "Requested By", "Approved By", "Repair PO #", "Status"] },
...
const assetColumnDefs = [
  ...
  ["make_model", "Make / Model"], ["color", "Color"], ["vin_serial", "VIN / Serial"], ["plate", "Plate"], ["location", "Location"], ["gps_location", "GPS"],
...
const ASSET_COLUMN_STORAGE_KEY = "lms.assetColumns.v5";
...
async function renderAccountingView() {
  ...
  const data = buildAccountingData({ gl, coa, invoices, invoiceLines, payments, pos, poLines, receipts, salesOrders, salesLines, bankRows, products, reportFrom, reportTo, reportTab: accountingTab });
...
async function renderAssetsView() {
  ...
  $("content").innerHTML = `
    ...
    <section class="panel fleet-panel">
      <div class="panel-head"><div class="panel-title"><strong>Vehicles & Heavy Equipment</strong><span>Ownership, readings, location, operator, and condition</span></div><div class="actions"><button id="assetFilterToggle">${assetFiltersCollapsed() ? "Show filters" : "Hide filters"}</button><button id="assetColumnsBtn">Show / Hide Columns</button><button id="assetTemplateBtn">Download asset template</button><button id="assetUploadBtn">Upload asset list</button><button id="assetCsvBtn">Excel</button><button onclick="window.print()">PDF / Print</button><button id="addLocationBtn">Add location</button><button id="assetQrCsvBtn">Excel + QR</button><button class="danger" id="clearFleetAssetsBtn">Clear assets</button><button class="primary" id="newAssetBtn">Add asset</button></div></div>

...
function truckingServiceSelectOptions(selected = "") {
  ...
  const values = [...new Set([...standard, ...(productMeta.truckingRates || []).map((rate) => rate.parent_service)]
...
async function saveTruckingRequest(previousNo = "") {
  ...
  const allowedServices = [...new Set([
    ...
    ...(productMeta.truckingRates || []).map((rate) => rate.service),
    ...(productMeta.truckingRates || []).map((rate) => rate.parent_service),
  ].filter(Boolean))];
...
function truckingRatesTable(rows) {
  ...
  return truckingSimpleTable(sorted.filter((row) => truckingRowMatchesSearch(row, $("truckRateSearch")?.value || "")), ["parent_service", "service", "rate", "rate_type", "additional", "category", "status"], {
    labels: ["Parent Service", "Rate Option / Service", "Rate", "Type", "Additional", "Category", "Status"],
...
async function openTruckingRateForm(service = "") {
  const row = service ? (await getAll("trucking_rates")).find((item) => item.service === service) : null;
  modalTitle.textContent = row ? `Edit rate ${service}` : "New trucking rate";
  modalBody.innerHTML = `
    <div class="form-grid">
      <label>Parent service<input name="parent_service" list="truckParentServiceOptions" value="${esc(row?.parent_service || "")}" placeholder="Example: Water Service"></label>
    ...
    <datalist id="truckParentServiceOptions">${["Equipment Move", "Container/Trailer Move", "High Deck Trailer", "Water Service", "Roll Off Service", "Pump Service"].map((value) => `<option value="${esc(value)}"></option>`).join("")}</datalist>

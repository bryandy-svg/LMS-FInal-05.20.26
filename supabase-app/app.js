...
function parseFuelAssetChoice(value) {
  ...
  return assets.find((asset) => fuelAssetLabel(asset).toLowerCase() === text.toLowerCase())
    || assets.find((asset) => String(asset.asset_tag || "").toLowerCase() === text.toLowerCase())
    || { source: "", asset_tag: text, equipment_name: text, asset_table: "", asset_key_field: "", asset_key: text };
}
...
async function updateFuelAssetLocationsFromFuel(lines, jobsite, fuelDate) {
  ...
  for (const line of lines || []) {
    const table = line._asset_table;
    const keyField = line._asset_key_field;
    const key = line._asset_key || line.asset_tag;
    if (!table || !keyField || !key) continue;
    const signature = `${table}:${keyField}:${key}`;
    if (updated.has(signature)) continue;
    updated.add(signature);
    const payload = table === "assets"
      ? { location, last_update_date: updateDate, scanned_date: stampedAt }
      : { location };
    const { error } = await supabase.from(table).update(payload).eq(keyField, key);
    if (!error) continue;
    if (table === "assets") await supabase.from(table).update({ location }).eq(keyField, key);
  }
}
...
async function renderTruckingRequestsView() {
  ...
  $("content").innerHTML = `
    <div class="toolbar ${isTruckingRequestorUser() ? "truck-mobile-toolbar" : ""}">
      <input class="searchbox" id="truckRequestSearch" placeholder="Search request, customer, project, jobsite, equipment, status">
    </div>
    <section class="panel">
      <div class="panel-head">
  ...
  enhanceSuggestInputs($("content"));
}

function truckingRequestTable(rows) {
  const filtered = rows.filter((row) => rowMatchesSearch(row, $("truckRequestSearch")?.value || ""));
  if (isTruckingRequestorUser()) {
    return `<div class="truck-mobile-list">${filtered.map((row) => `
      <article class="truck-mobile-card">
        <div><strong>${esc(row.request_no)}</strong><span class="badge">${esc(row.status || "Requested")}</span></div>
        <p>${esc(row.customer || "No customer")} | ${esc(row.jobsite || row.project || "No jobsite")}</p>
        <small>${esc(row.request_date || "")} | ${esc(row.line_count || 0)} equipment line${Number(row.line_count || 0) === 1 ? "" : "s"}</small>
      </article>`).join("") || `<div class="empty">No move requests yet.</div>`}</div>`;
  }
  return simpleTable(filtered, ["request_no", "request_date", "customer", "project", "jobsite", "requested_by", "requestor_email", "needed_date", "line_count", "status"], {
    labels: ["Request #", "Date", "Customer", "Project", "Jobsite", "Requested By", "Email", "Needed Date", "Lines", "Status"],
...
async function openTruckingRequestForm(requestNo = "") {
  ...
  modalBody.innerHTML = `
    <div class="form-grid">
      ...
      <label>Status<select name="status"><option>Requested</option><option>Dispatched</option><option>Completed</option><option>Cancelled</option></select></label>
    </div>
    ...
    ${truckingDatalists()}`;
  modalBody.querySelector("[name=status]").value = existing?.status || "Requested";
  ...
  $("addTruckRequestLineBtn").onclick = () => {
    ...
    enhanceSuggestInputs($("truckRequestLines"));
  };
  openModal();
  enhanceSuggestInputs(modalBody);
}
...
async function saveTruckingRequest(previousNo = "") {
  ...
  const lines = [...modalBody.querySelectorAll(".truck-request-line")].map((row, index) => {
    ...
    const asset = (productMeta.truckingAssets || []).find((item) => item.label === equipment) || {};
    return {
...
function truckingMoveTable(rows) {
  const filtered = rows.filter((row) => rowMatchesSearch(row, $("truckMoveSearch")?.value || ""));
  if (isTruckingDriverUser()) {
    return `<div class="truck-mobile-list">${filtered.map((row) => `
      <article class="truck-mobile-card">
  ...
  return simpleTable(filtered, ["ticket_no", "move_date", "customer", "project", "jobsite", "driver_name", "service", "equipment_label", "origin", "destination", "container_no", "start_time", "end_time", "amount", "status"], {
...
function truckingRatesTable(rows) {
  return simpleTable(rows.filter((row) => rowMatchesSearch(row, $("truckRateSearch")?.value || "")), ["service", "rate", "rate_type", "additional", "category", "status"], {

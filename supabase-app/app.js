...
async function saveAssetModal() {
  ...
  try {
    ...
    let saved;
    try {
      saved = await upsertOne("assets", record, "asset_tag");
    } catch (error) {
      if (isMissingSchemaColumn(error, "compatible_with")) {
        const retryRecord = { ...record };
        delete retryRecord.compatible_with;
        saved = await upsertOne("assets", retryRecord, "asset_tag");
        alert("Saved the asset. To store the Compatible With field too, run the latest asset database update in Supabase.");
      } else {
        throw error;
      }
    }

...
const modules = [
  ...
  { group: "Trucking", items: [["truckingrequests", "Move Requests"], ["trucking", "Dispatcher Scheduler"], ["truckingrates", "Rate Sheet"]] },
    ...
    : isEquipmentRequestUser()
      ...
      : isFuelUser()
        ...
        : isTruckingDriverUser()
          ? [{ group: "Trucking Driver", items: [["trucking", "My Moves"]] }]
...
function isTruckingDriverUser() {
  ...
  const mods = userModules();
  const visible = mods.filter((m) => m && m !== "dashboard");
  return /driver/i.test(role) || (!mods.includes("all") && (visible.includes("trucking_driver") || (visible.length === 1 && visible[0] === "trucking")));
}
...
function defaultView() {
  ...
  if (isTruckingDriverUser()) return "trucking";
...
function canAccess(view) {
  ...
  if (isTruckingDriverUser()) return view === "trucking";
...
async function loadView(view) {
  ...
  if (isTruckingDriverUser()) view = "trucking";
...
async function loadTruckingLookups() {
  ...
  productMeta.truckingDrivers = (users || [])
    .filter((user) => /driver/i.test(`${user.role || ""} ${user.modules || ""}`))
...
async function openTruckingDispatchFromRequest(requestNo) {
  ...
  modalBody.innerHTML = `
    ...
    <div class="truck-lines">
      <div class="mini-grid truck-line-head"><strong>Include</strong><strong>Equipment</strong><strong>Service</strong><strong>Origin</strong><strong>Destination</strong></div>
...
async function saveTruckingDispatch(req, reqLines) {
  ...
  for (const row of [...modalBody.querySelectorAll(".truck-dispatch-line")]) {
    ...
    moveRows.push({
      ...
      po_no: req.po_no,
      asset_source: original.asset_source,
      asset_tag: original.asset_tag,
      equipment_label: original.equipment_label,
      service,
      trip_type: tripType,
      ...
      origin: inputs[2]?.value.trim() || original.origin || "",
      destination: inputs[3]?.value.trim() || original.destination || "",
...
async function printTruckingTicket(ticketNo) {
  ...
  const html = `<!doctype html><html><head><title>Ticket ${esc(row.ticket_no)}</title><style>
    body{font-family:Arial,Helvetica,sans-serif;color:#000;margin:0;padding:10px}.ticket{max-width:1050px;margin:auto;border:2px solid #111}.head{display:grid;grid-template-columns:1fr 1fr 240px;gap:10px;padding:8px;border-bottom:2px solid #111}.logo{font-size:42px;font-weight:900;font-style:italic}.red{color:#d00}.checkrow{display:grid;grid-template-columns:repeat(6,1fr);gap:16px;padding:8px}.boxgrid{display:grid;grid-template-columns:1.15fr 1fr;border-top:2px solid #111}.cell{min-height:38px;border-bottom:1px solid #111;padding:4px 6px}.right{border-left:1px solid #111}.label{font-weight:900}.fill{background:#eaf1fb;font-style:italic;font-size:18px}.sign{height:80px}.small{font-size:12px}.sigline{border-top:1px solid #111;margin-top:28px;padding-top:3px}@media print{@page{size:Letter landscape;margin:.25in}body{padding:0}.ticket{max-width:none}}</style></head><body>
    <div class="ticket">
      <div class="head"><div><h2>CONTAINER DELIVERY RECEIPT</h2><h2>TICKET NO: <span class="red">${esc(row.ticket_no)}</span></h2></div><div class="small">Domestic [ ] One-Way [${row.trip_type !== "Round Trip" ? "X" : " "}] Export [ ]<br>Foreign [ ] Round Trip [${row.trip_type === "Round Trip" ? "X" : " "}] Respot [ ]</div><div><div class="logo">LMSGU</div><b>202 Hilton Rd. Tasi Towers B01</b><br>Tumon Bay, Guam 96913<br>Tel: (671) 647-2617<br>Email: trucking@lmsfm.com</div></div>
      <div class="boxgrid">
        <div><div class="cell label">Date:</div><div class="cell fill">${esc(row.move_date || "")}</div><div class="cell label">Origin:</div><div class="cell fill">${esc(row.origin || "")}</div><div class="cell label">Destination: &nbsp;&nbsp;&nbsp; Project:</div><div class="cell fill">${esc(row.destination || "")} &nbsp;&nbsp;&nbsp; ${esc(row.project || row.jobsite || "")}</div><div class="cell label">Container No.:</div><div class="cell fill">${esc(row.container_no || "")}</div><div class="cell label">Vessel/Voyage:</div><div class="cell fill">${esc(row.vessel_voyage || "")}</div><div class="cell label">Customer Print:</div><div class="cell fill">${esc(row.customer_print || row.customer || "")}</div><div class="cell label">Customer Signature: <span style="float:right">Date:</span></div><div class="cell sign">${esc(row.customer_signature || "")}</div></div>
        <div class="right"><div class="cell label">Customer:</div><div class="cell fill">${esc(row.customer || "")}</div><div class="cell label">Driver:</div><div class="cell fill">${esc(row.driver_name || "")}</div><div class="cell label">Start Time: <span style="float:right">End Time:</span></div><div class="cell fill">${esc(row.start_time || "")}<span style="float:right">${esc(row.end_time || "")}</span></div><div class="cell label">Seal No.:</div><div class="cell fill">${esc(row.seal_no || "")}</div><div class="cell label">BL No.:</div><div class="cell fill">${esc(row.bl_no || "")}</div><div class="cell label">Truck License: <span style="float:right">Chassis No.:</span></div><div class="cell fill">${esc(row.truck_license || "")}<span style="float:right">${esc(row.chassis_no || "")}</span></div><div class="cell label">Driver Print:</div><div class="cell fill">${esc(row.driver_print || row.driver_name || "")}</div><div class="cell label">Driver Signature: <span style="float:right">Date:</span></div><div class="cell sign">${esc(row.driver_signature || "")}</div></div>
      </div>

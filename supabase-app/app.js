...
const tableMap = {
  ...
  truckingrequests: { table: "trucking_requests", key: "request_no", title: "Move Requests", sub: "Request trucking moves with configurable service patterns, rates, locations, and equipment details.", heads: ["request_no", "request_date", "customer", "project_jobsite", "contact_no", "needed_date", "delivery_period", "trip_type", "service_needed", "service_subcategory", "service_size", "service_reference", "material_description", "unit_of_measurement", "weight_lb", "dimensions", "materials_loaded", "number_of_loads", "cy_per_load", "total_cy", "tipping_fee", "origin", "destination", "requested_by", "requestor_email", "notes"], labels: ["Request #", "Date", "Customer", "Project / Jobsite", "Contact #", "Requested Delivery", "AM / PM", "Trip Type", "Super Parent", "Move Type / Option", "Size / Capacity", "Reference #", "Material(s)", "Unit of Measurement", "Weight (lb)", "Dimensions", "Materials Loaded", "# of Loads", "CY / Load", "Total CY", "Tipping Fee", "Origin", "Destination", "Requested By", "Email", "Move Description"] },
...
function truckingRequestTable(rows) {
  ...
  return truckingSimpleTable(filtered, ["request_no", "request_date", "customer", "project_jobsite", "contact_no", "needed_date", "delivery_period", "trip_type", "service_needed", "service_subcategory", "service_size", "service_reference", "material_description", "unit_of_measurement", "weight_lb", "dimensions", "materials_loaded", "number_of_loads", "cy_per_load", "total_cy", "tipping_fee", "origin", "destination", "requested_by", "line_count", "notes"], {
    labels: ["Request #", "Date", "Customer", "Project / Jobsite", "Contact #", "Requested Delivery", "AM / PM", "Trip Type", "Super Parent", "Move Type / Option", "Size / Capacity", "Reference #", "Material(s)", "Unit of Measurement", "Weight (lb)", "Dimensions", "Materials Loaded", "# of Loads", "CY / Load", "Total CY", "Tipping Fee", "Origin", "Destination", "Requested By", "Lines", "Move Description"],
...
async function openTruckingRequestForm(requestNo = "") {
  ...
  modalBody.innerHTML = `
    ...
    <div class="form-grid">
      ...
      <label>Origin<input class="suggest-input" name="origin" list="truckJobsiteOptions" value="${esc(existing?.origin || "")}" placeholder="Search or enter a new origin" autocomplete="off" required></label>
      <label>Destination<input class="suggest-input" name="destination" list="truckJobsiteOptions" value="${esc(existing?.destination || "")}" placeholder="Search or enter a new destination" autocomplete="off" required></label>
    ...
    <label class="move-request-description">Move description<textarea name="notes" placeholder="Describe what needs to be moved, where, timing, and special instructions.">${esc(existing?.notes || "")}</textarea></label>
    ${truckingDatalists()}
    </div>`;
...
async function saveTruckingRequest(previousNo = "") {
  ...
  if (!values.origin) return alert("Origin is required.");
  if (!values.destination) return alert("Destination is required.");
  ...
  for (const row of [...modalBody.querySelectorAll(".truck-dispatch-line")]) {
    ...
    moveRows.push({
      ...
      notes: [
        ...
        `${tripType} charge: ${tripMultiplier}×`,
      ].filter(Boolean).join(" | "),
      status: "Scheduled",
    });
...
async function saveTruckingMove() {
  ...
  if (rate) {
    ...
    payload.amount = Number(rate.rate || 0) * (payload.trip_type === "Round Trip" ? 2 : 1);
  }

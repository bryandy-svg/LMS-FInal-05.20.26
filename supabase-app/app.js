...
async function clearFleetAssetsOnly() {
  ...
  try {
    await requireDb();
    await supabase.from("work_orders").update({ asset_id: null }).not("asset_id", "is", null);
    await supabase.from("rentals").update({ item_id: null }).eq("item_type", "Asset");
    await supabase.from("equipment_requests").update({ asset_id: null }).not("asset_id", "is", null);

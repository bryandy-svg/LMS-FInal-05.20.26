...
function parseFuelAssetChoice(value) {
  ...
  return assets.find((asset) => fuelAssetLabel(asset).toLowerCase() === text.toLowerCase())
    ...
    || { source: "", asset_tag: text, equipment_name: text };
}
...
async function saveFuelReport() {
  ...
  try {
    ...
    const { error } = await supabase.from("fuel_logs").insert(lines);
    if (error) throw error;
...
function openExcelFilterMenu(button) {
  ...
  popup.innerHTML = `
    ...
    <div class="excel-filter-footer">
      ...
      <button type="button" class="primary" data-apply>Apply</button>
    </div>`;

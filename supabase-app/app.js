...
function updateTruckingPatternFields(root, service, selected = {}) {
  ...
  if (rollOffTonsInput) {
    rollOffTonsInput.required = needsTippingFee;
...
async function openTruckingRequestForm(requestNo = "") {
  ...
  modalBody.innerHTML = `
    ...
    <div class="form-grid">
      ...
      <label id="truckRollOffTonsField" hidden>Tons<input type="number" min="0.01" step="0.01" name="roll_off_tons" value="${esc(existing?.roll_off_tons || "")}" placeholder="Actual or estimated tons"><small>Per-ton tipping rate × tons.</small></label>
...
async function saveTruckingRequest(previousNo = "") {
  ...
  if (values.service_needed.toLowerCase() === "roll off service" && !Number(values.roll_off_tons || 0)) return alert("Enter the tons for Roll Off Service.");
  if (["high deck trailer", "highbed/chameleon", "flatrack/chameleon"].includes(values.service_needed.toLowerCase())) {
...
async function renderTruckingTicketsView() {
  ...
  const render = () => {
    ...
    const heads = truckingTicketBillingTab === "unbilled"
      ? ["billing_select", "ticket_no", "move_date", "customer", "driver_name", "equipment_label", "origin", "destination", "service", "start_time", "end_time", "move_description", "amount", "discount_amount", "amount_to_bill", "billing_status"]
      : ["ticket_no", "move_date", "customer", "driver_name", "equipment_label", "origin", "destination", "service", "start_time", "end_time", "move_description", "amount", "discount_amount", "amount_to_bill", "billing_status", "billing_reference"];
    const labels = truckingTicketBillingTab === "unbilled"
      ? ["Combine", "Ticket #", "Dispatch Date", "Customer", "Driver", "Actual Equipment", "Origin", "Destination", "Service", "Start Time", "End Time", "Move Description", "Original Charge", "Discount", "Amount to Bill", "Billing Status"]
      : ["Ticket #", "Dispatch Date", "Customer", "Driver", "Actual Equipment", "Origin", "Destination", "Service", "Start Time", "End Time", "Move Description", "Original Charge", "Discount", "Amount to Bill", "Billing Status", "External Billing Reference"];
...
function truckingCustomerBillingSummaryTable(rows) {
  ...
  for (const row of rows) {
    ...
    const group = groups.get(key) || { customer, billing_batch, tickets: [], original: 0, discount: 0, billed: 0, billing_reference: "" };
  ...
  return truckingSimpleTable(summary, ["billing_batch", "customer", "ticket_count", "original", "discount", "billed", "billing_reference"], {
    labels: ["Billing Group", "Customer", "Tickets", "Original Charges", "Discounts", "Amount to Be Billed", "External Billing Reference"],
    format: {
...
async function openTruckingCustomerBillingSummary(customer, billingBatch = "") {
  ...
  modalBody.innerHTML = `
    ...
    <div class="tablewrap billing-discount-table"><table>
      <thead><tr><th>Ticket</th><th>Date</th><th>Description of Move</th><th>Original Charge</th><th>Discount</th><th>Amount to Bill</th></tr></thead>
      <tbody>${rows.map((row) => `<tr data-billing-ticket="${esc(row.ticket_no)}" data-original="${esc(Number(row.amount || 0))}">
        <td>${esc(row.ticket_no)}</td><td>${esc(row.move_date || "")}</td><td>${esc(row.move_description || row.notes || "")}</td><td>${money(row.amount || 0)}</td>
...
async function printTruckingCustomerBillingSummary(customer, selectedTicketNos = [], targetWindow = null) {
  ...
  const ticketPages = rows.map((row) => `
    <section class="ticket-page"><img class="brand" src="${esc(headerUrl)}" alt="Landscape Management Systems, Inc.">
      ...
      <div class="ticket-grid">
        ...
        <div><b>Original Charge</b><span>${money(row.amount || 0)}</span></div><div><b>Discount</b><span>${money(row.discount_amount || 0)}</span></div>
  ...
  const html = `<!doctype html><html><head><title>Billing Summary - ${esc(customer)}</title><style>
    @page{size:Letter landscape;margin:.35in}*{box-sizing:border-box}body{font-family:Arial,Helvetica,sans-serif;color:#111;margin:0}.sheet{width:100%;page-break-inside:avoid}.brand{width:100%;max-height:1.05in;object-fit:contain;display:block;margin-bottom:8px}.title{display:flex;justify-content:space-between;align-items:flex-end;border-bottom:2px solid #111;padding-bottom:8px;margin-bottom:10px}.title h1{font-size:22px;margin:0}.title div{text-align:right}table{width:100%;border-collapse:collapse;font-size:8px;table-layout:fixed}th,td{border:1px solid #777;padding:4px;text-align:left;overflow-wrap:anywhere}th{background:#e9eef1;text-transform:uppercase;font-size:7px}.num{text-align:right}.totals{width:44%;margin:12px 0 0 auto}.totals div{display:flex;justify-content:space-between;border-bottom:1px solid #aaa;padding:6px}.totals .due{font-size:16px;font-weight:900;border:2px solid #111}.muted{color:#555}.ticket-page{page-break-before:always;width:100%;page-break-inside:avoid}.ticket-grid{display:grid;grid-template-columns:1fr 1fr;border:1px solid #777}.ticket-grid>div{padding:7px;border-bottom:1px solid #aaa}.ticket-grid>div:nth-child(odd):not(.wide){border-right:1px solid #aaa}.ticket-grid b,.ticket-grid span{display:block}.ticket-grid span{margin-top:4px}.ticket-grid .wide{grid-column:1/-1}.ticket-grid .bill{font-size:18px;display:flex;justify-content:space-between}.signature-row{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:12px}.signature-row>div{border:1px solid #777;padding:8px}.signature-row img{display:block;width:100%;height:90px;object-fit:contain;margin-top:6px}</style></head><body>
    <main class="sheet"><img class="brand" src="${esc(headerUrl)}" alt="Landscape Management Systems, Inc.">
      ...
      <table><thead><tr><th>Ticket #</th><th>Date</th><th>Start</th><th>End</th><th>Service</th><th>Description of Move</th><th>Origin</th><th>Destination</th><th>Driver</th><th>Equipment</th><th>Payment</th><th>PO #</th><th class="num">Original</th><th class="num">Discount</th><th class="num">Amount to Bill</th></tr></thead>
      ...
      <div class="totals"><div><span>Original Charges</span><strong>${money(originalTotal)}</strong></div><div><span>Total Discounts</span><strong>${money(discountTotal)}</strong></div><div class="due"><span>Amount to Be Billed</span><strong>${money(billedTotal)}</strong></div></div>
...
async function saveTruckingDispatch(req, reqLines) {
  ...
  for (const row of [...modalBody.querySelectorAll(".truck-dispatch-line")]) {
    ...
    const tippingMultiplier = /ton/i.test(String(tippingRate.rate_type || "")) ? Number(req.roll_off_tons || 0) : 1;
    const tippingCharge = req.service_needed === "Roll Off Service" ? Number(tippingRate.rate || 0) * tippingMultiplier : 0;
    const originalCharge = Number(rate.rate || 0) * volumeMultiplier * tripMultiplier + tippingCharge;
    ...
    moveRows.push({
      ...
      notes: [
        ...
        tripType === "Round Trip" && isHourlyRate ? "Round Trip: no 2× multiplier on hourly rate" : `${tripType} charge: ${tripMultiplier}×`,
      ].filter(Boolean).join(" | "),
      status: "Scheduled",
    });
...
async function saveTruckingMove() {
  ...
  if (rate) {
    ...
    if (!Number(payload.amount || 0)) payload.amount = Number(rate.rate || 0) * (payload.trip_type === "Round Trip" && !/hour/i.test(String(rate.rate_type || "")) ? 2 : 1);
  }
...
async function printTruckingTicket(ticketNo) {
  ...
  const html = `<!doctype html><html><head><title>Ticket ${esc(row.ticket_no)}</title><style>
    body{font-family:Arial,Helvetica,sans-serif;color:#000;margin:0;padding:10px}.ticket{width:100%;max-width:1050px;box-sizing:border-box;margin:auto;border:2px solid #111;break-inside:avoid}.ticket-brand{padding:10px;border-bottom:2px solid #111}.ticket-brand img{display:block;width:100%;height:auto}.head{display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:8px;border-bottom:2px solid #111}.red{color:#d00}.checkrow{display:grid;grid-template-columns:repeat(6,1fr);gap:16px;padding:8px}.boxgrid{display:grid;grid-template-columns:1.15fr 1fr;border-top:2px solid #111}.cell{min-height:38px;border-bottom:1px solid #111;padding:4px 6px}.right{border-left:1px solid #111}.label{font-weight:900}.fill{background:#eaf1fb;font-style:italic;font-size:18px}.sign{height:80px}.sign img{display:block;max-width:100%;max-height:76px;object-fit:contain}.small{font-size:12px}.sigline{border-top:1px solid #111;margin-top:28px;padding-top:3px}@media print{@page{size:Letter landscape;margin:.3in}html,body{width:100%;height:auto}body{padding:0}.ticket{width:100%;max-width:none;page-break-inside:avoid}.ticket-brand img{max-height:1.05in;object-fit:contain}.cell{min-height:29px;font-size:12px;padding:3px 5px}.fill{font-size:14px}.sign{height:58px}.sign img{max-height:54px}.head h2{margin:4px 0}}</style></head><body>
    <div class="ticket">
      ...
      <div class="boxgrid">
        ...
        <div class="right"><div class="cell label">Customer:</div><div class="cell fill">${esc(row.customer || "")}</div><div class="cell label">Driver:</div><div class="cell fill">${esc(row.driver_name || "")}</div><div class="cell label">Start Time: <span style="float:right">End Time:</span></div><div class="cell fill">${esc(row.start_time || "")}<span style="float:right">${esc(row.end_time || "")}</span></div><div class="cell label">Service:</div><div class="cell fill">${esc(row.service || "")}</div><div class="cell label">Original Charge:</div><div class="cell fill">${money(row.amount || 0)}</div><div class="cell label">Discount:</div><div class="cell fill">${money(row.discount_amount || 0)}</div><div class="cell label">Amount to Be Billed:</div><div class="cell fill"><strong>${money(row.amount_to_bill ?? row.amount ?? 0)}</strong></div><div class="cell label">Driver Print:</div><div class="cell fill">${esc(row.driver_print || row.driver_name || "")}</div><div class="cell label">Driver Signature: <span style="float:right">Date:</span></div><div class="cell sign">${signatureHtml(row.driver_signature)}</div></div>
      </div>

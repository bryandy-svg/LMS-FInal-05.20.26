...
function checkPrintHtml(row, vendor, invoices, amount, userNotes = "") {
  ...
  const invoiceRows = invoices.map((inv) => `<tr>
    ...
    <td>${esc(inv.parts_description || "N/A")}</td>
  </tr>`).join("");
  return `<html><head><title>Check ${esc(checkNo)}</title><style>
    ...
    *{box-sizing:border-box}
    ...
    .voucher-table th:nth-child(1){width:.9in}.voucher-table th:nth-child(2){width:.82in}.voucher-table th:nth-child(3){width:.78in}.voucher-table th:nth-child(4){width:.85in}.voucher-table th:nth-child(5){width:.8in}.voucher-table th:nth-child(6){width:1.15in}.voucher-table th:nth-child(7){width:1.25in}
    ...
    <section class="voucher-top">
      ...
      <table class="voucher-table">
        <thead><tr><th>Invoice #</th><th>Invoice Date</th><th>Amount</th><th>Work Order #</th><th>PO #</th><th>Jobsite</th><th>Equipment</th><th>Description of Parts</th></tr></thead>
...
function uploadBankTransactionsFile(defaultBank = "") {
  ...
  input.onchange = async () => {
    ...
    try {
      ...
      alert(`${rows.length} bank transaction row${rows.length === 1 ? "" : "s"} uploaded for reconciliation.`);
...
function signatureBlockHtml(title = "Customer acceptance", savedSignature = "") {
  const customerSignature = savedSignature
    ? `<img class="saved-signature" src="${esc(savedSignature)}" alt="Saved signature"><div class="sig-actions"><span>Saved signature prints on this document</span></div>`
    : `<canvas class="sig-pad" width="560" height="150" data-signature-pad></canvas><div class="sig-actions"><button type="button" data-clear-signature>Clear signature</button><span>Sign online before printing or saving PDF</span></div>`;

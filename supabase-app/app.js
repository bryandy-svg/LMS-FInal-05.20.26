...
const tableMap = {
  ...
  truckingrequests: { table: "trucking_requests", key: "request_no", title: "Move Requests", sub: "Request trucking moves with delivery timing, service, origin, destination, and equipment details before dispatch scheduling.", heads: ["request_no", "request_date", "customer", "project_jobsite", "contact_no", "needed_date", "delivery_period", "service_needed", "origin", "destination", "requested_by", "requestor_email", "notes"], labels: ["Request #", "Date", "Customer", "Project / Jobsite", "Contact #", "Requested Delivery", "AM / PM", "Service Needed", "Origin", "Destination", "Requested By", "Email", "Move Description"] },
  ...
  truckingrates: { table: "trucking_rates", key: "service", title: "Trucking Rate Sheet", sub: "Service rates, tipping fees, hourly/flat/daily rules, and billing support.", heads: ["service", "rate", "rate_type", "additional", "category", "status"], labels: ["Service", "Rate", "Type", "Additional", "Category", "Status"] },
...
function truckingServiceSelectOptions(selected = "") {
  ...
  const values = [...new Set([...standard, ...(productMeta.truckingRates || []).map((rate) => rate.service)]
...
function truckingRequestTable(rows) {
  ...
  return truckingSimpleTable(filtered, ["request_no", "request_date", "customer", "project_jobsite", "contact_no", "needed_date", "delivery_period", "service_needed", "origin", "destination", "requested_by", "line_count", "notes"], {
    labels: ["Request #", "Date", "Customer", "Project / Jobsite", "Contact #", "Requested Delivery", "AM / PM", "Service Needed", "Origin", "Destination", "Requested By", "Lines", "Move Description"],
...
async function openTruckingRequestForm(requestNo = "") {
  ...
  modalBody.innerHTML = `
    ...
    <div class="truck-lines" id="truckRequestLines">
      <div class="mini-grid truck-line-head" style="grid-template-columns:minmax(220px,2fr) minmax(180px,1fr)"><strong>Equipment</strong><strong>Notes</strong></div>
      ${lineRows.map((line) => truckingRequestLineHtml(line)).join("")}
    </div>
    <button type="button" id="addTruckRequestLineBtn">Add equipment row</button>
    <label>Description of move requested<textarea name="notes" placeholder="Describe what needs to be moved, where, timing, and special instructions.">${esc(existing?.notes || "")}</textarea></label>
    ${truckingDatalists()}`;
...
function truckingRequestLineHtml(line) {
  return `<div class="mini-grid truck-request-line" style="grid-template-columns:minmax(220px,2fr) minmax(180px,1fr)">
...
function truckingRatesTable(rows) {
  return truckingSimpleTable(rows.filter((row) => truckingRowMatchesSearch(row, $("truckRateSearch")?.value || "")), ["service", "rate", "rate_type", "additional", "category", "status"], {
    labels: ["Service", "Rate", "Type", "Additional", "Category", "Status"],
...
async function openTruckingRateForm(service = "") {
  ...
  modalBody.innerHTML = `
    <div class="form-grid">
      <label>Service<input name="service" value="${esc(row?.service || "")}"></label>

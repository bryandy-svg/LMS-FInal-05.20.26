...
create table if not exists purchase_orders (
  ...
  spending_limit numeric not null default 0 check (spending_limit >= 0),
  payment_terms text,
  incoterm text,
  ...
  posting_date date,
  purchase_purpose text not null default 'Inventory Stock' check (purchase_purpose in ('Inventory Stock', 'Customer Order', 'Work Order')),
  linked_customer text,
  linked_sales_order_no text,
  ap_support_wo_no text,
  ap_support_jobsite text,
  ap_support_equipment text,
  ap_support_parts_description text,
...
create table if not exists sales_orders (
  ...
  order_type text not null default 'Stock Order',
  special_order_status text not null default 'Unfulfilled',
  deposit_invoice_no text,
  deposit_amount numeric not null default 0,
  deposit_posted_at timestamptz,
...
begin
  foreach t in array array[
    ...
    'stock_movements','purchase_orders','purchase_order_lines','goods_receipts','quotations','quotation_lines','sales_orders','sales_order_lines','asset_locations','asset_types',
    'assets','equipment_requests','outside_customer_fleet','mechanics','work_orders','work_order_issues','work_order_parts','work_order_labor','rentals','invoices','invoice_lines',
